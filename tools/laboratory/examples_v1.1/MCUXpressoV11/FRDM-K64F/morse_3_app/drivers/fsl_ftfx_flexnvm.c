...
status_t FLEXNVM_Init(flexnvm_config_t *config) {
    ...
    config->ftfxConfig.opsConfig.addrAligment.blockWriteUnitSize =
        FSL_FEATURE_FLASH_FLEX_NVM_BLOCK_WRITE_UNIT_SIZE;
    config->ftfxConfig.opsConfig.addrAligment.sectorCmd =
        FSL_FEATURE_FLASH_FLEX_NVM_SECTOR_CMD_ADDRESS_ALIGMENT;
    config->ftfxConfig.opsConfig.addrAligment.sectionCmd =
        FSL_FEATURE_FLASH_FLEX_NVM_SECTION_CMD_ADDRESS_ALIGMENT;
    config->ftfxConfig.opsConfig.addrAligment.resourceCmd =
        FSL_FEATURE_FLASH_FLEX_NVM_RESOURCE_CMD_ADDRESS_ALIGMENT;
    config->ftfxConfig.opsConfig.addrAligment.checkCmd =
        FSL_FEATURE_FLASH_FLEX_NVM_CHECK_CMD_ADDRESS_ALIGMENT;

    /* Set Flexnvm memory properties */
    config->ftfxConfig.flashDesc.blockBase =
        FSL_FEATURE_FLASH_FLEX_NVM_START_ADDRESS;
#if defined(FSL_FEATURE_FLASH_HAS_FLEX_NVM_ALIAS) &&                           \
    FSL_FEATURE_FLASH_HAS_FLEX_NVM_ALIAS
    config->ftfxConfig.flashDesc.aliasBlockBase =
    ...
    config->ftfxConfig.flashDesc.blockCount =
        FSL_FEATURE_FLASH_FLEX_NVM_BLOCK_COUNT;
...
status_t FLEXNVM_DflashProgramSection(flexnvm_config_t *config, uint32_t start,
    ...
    return FTFx_CMD_ProgramSection(&config->ftfxConfig, start, src,
                                   lengthInBytes);
}
...
status_t FLEXNVM_ProgramPartition(flexnvm_config_t *config,
    ...
    return FTFx_CMD_ProgramPartition(&config->ftfxConfig, option,
...
status_t FLEXNVM_ReadResource(flexnvm_config_t *config, uint32_t start,
    ...
    return FTFx_CMD_ReadResource(&config->ftfxConfig, start, dst, lengthInBytes,
                                 option);
}
...
status_t FLEXNVM_DflashVerifyErase(flexnvm_config_t *config, uint32_t start,
    ...
    return FTFx_CMD_VerifyErase(&config->ftfxConfig, start, lengthInBytes,
                                margin);
}
...
status_t FLEXNVM_DflashVerifyProgram(flexnvm_config_t *config, uint32_t start,
    ...
    return FTFx_CMD_VerifyProgram(&config->ftfxConfig, start, lengthInBytes,
                                  expectedData, margin, failedAddress,
                                  failedData);
}
...
status_t FLEXNVM_EepromWrite(flexnvm_config_t *config, uint32_t start,
    ...
    if ((start < config->ftfxConfig.flexramBlockBase) ||
        ((start + lengthInBytes) > (config->ftfxConfig.flexramBlockBase +
    ...
    if (0U == (FTFx->FCNFG & FTFx_FCNFG_EEERDY_MASK)) {
        ...
        returnCode = FTFx_CMD_SetFlexramFunction(
            &config->ftfxConfig, kFTFx_FlexramFuncOptAvailableForEeprom);
        if (returnCode != kStatus_FTFx_Success) {
    ...
    while (lengthInBytes > 0U) {
        if ((0U == (start & 0x3U)) && (0U == ((uint32_t)src & 0x3U)) &&
            (lengthInBytes >= 4U)) {
            *(uint32_t *)start = *(uint32_t *)(uint32_t)src;
            start += 4U;
            src = &src[4];
            lengthInBytes -= 4U;
        } else if ((0U == (start & 0x1U)) && (0U == ((uint32_t)src & 0x1U)) &&
                   (lengthInBytes >= 2U)) {
...
status_t FLEXNVM_DflashSetProtection(flexnvm_config_t *config,
    ...
    if ((config->ftfxConfig.flashDesc.totalSize == 0U) ||
        (config->ftfxConfig.flashDesc.totalSize == 0xFFFFFFFFU)) {
        return kStatus_FTFx_CommandNotSupported;
    }

    /* Individual data flash regions will be protected from program and erase
     * operations by setting the associated DPROT bit to the protected state.
     * Each FDPROT bit only be changed from 1s to 0s,
...
status_t FLEXNVM_EepromSetProtection(flexnvm_config_t *config,
    ...
    if ((config->ftfxConfig.eepromTotalSize == 0U) ||
        (config->ftfxConfig.eepromTotalSize == 0xFFFFU)) {
        return kStatus_FTFx_CommandNotSupported;
    }
    /* Individual data flash regions will be protected from program and erase
     * operations by setting the associated FEPROT bit to the protected state;
     * Each FEPROT bit only be changed from 1s to 0s,
     * meaning the protection can only be increased. */
...
static status_t flexnvm_convert_start_address(flexnvm_config_t *config,
    ...
    /* From Spec: When required by the command, address bit 23 selects between
     * program flash memory
     * (=0) and data flash memory (=1).*/
    if (start >= config->ftfxConfig.flashDesc.blockBase) {
        config->ftfxConfig.opsConfig.convertedAddress =
            start - config->ftfxConfig.flashDesc.blockBase + 0x800000U;
    ...
    /* for MKW39/38/37 which have alias blockBase */
        ...
        config->ftfxConfig.opsConfig.convertedAddress =
            start - config->ftfxConfig.flashDesc.aliasBlockBase + 0x800000U;
        status = kStatus_FTFx_Success;
    } else {
        status = kStatus_FTFx_Success;
    }
