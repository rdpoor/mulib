...
#if defined(MSCM_OCMDR_OCMC1_MASK) || defined(MSCM_OCMDR_OCM1_MASK) ||         \
    defined(MSCM_OCMDR0_OCM1_MASK) || defined(MSCM_OCMDR1_OCM1_MASK)
#define FLASH_PREFETCH_SPECULATION_IS_CONTROLLED_BY_MSCM (1u)
#else
#define FLASH_PREFETCH_SPECULATION_IS_CONTROLLED_BY_MSCM (0u)
#endif

#if defined(FMC_PFB0CR_S_INV_MASK) || defined(FMC_PFB0CR_S_B_INV_MASK) ||      \
    defined(FMC_PFB01CR_S_INV_MASK) || defined(FMC_PFB01CR_S_B_INV_MASK)
...
#define FLASH_PREFETCH_SPECULATION_IS_CONTROLLED_BY_FMC (0u)
    ...
    FLASH_CACHE_IS_CONTROLLED_BY_MSCM
...
void mscm_flash_prefetch_speculation_enable(ftfx_cache_config_t *config,
                                            bool enable);
...
static void ftfx_common_bit_operation_command_sequence(
    ftfx_cache_config_t *config, FTFx_REG32_ACCESS_TYPE base, uint32_t bitMask,
...
static void
ftfx_copy_common_bit_operation_to_ram(uint32_t *ftfxCommonBitOperation);
#endif /* FTFx_DRIVER_IS_FLASH_RESIDENT */
 ...
 * @code
 *   void ftfx_common_bit_operation(FTFx_REG32_ACCESS_TYPE base, uint32_t
 * bitMask, uint32_t bitShift, uint32_t bitValue)
 *   {
 *       if (bitMask)
 *       {
 *           uint32_t value = (((uint32_t)(((uint32_t)(bitValue)) << bitShift))
 * & bitMask); *base = (*base & (~bitMask)) | value;
 *       }
 *
 *       __ISB();
 *       __DSB();
...
static const uint32_t s_ftfxCommonBitOperationFunctionCode[] = {
    0x2900b510u, 0x6804d005u, 0x4093438cu, 0x43214019u,
    0xf3bf6001u, 0xf3bf8f6fu, 0xbd108f4fu,
};
...
/* copy required flash commands to RAM */
    ...
    ftfx_copy_common_bit_operation_to_ram(
        (uint32_t *)config->bitOperFuncAddr.commadAddr);
#endif /* FLASH_IS_CACHE_INVALIDATION_AVAILABLE &&                             \
          FTFx_DRIVER_IS_FLASH_RESIDENT */
...
status_t FTFx_CACHE_ClearCachePrefetchSpeculation(ftfx_cache_config_t *config,
                                                  bool isPreProcess) {
    /* We pass the ftfx register address as a parameter to
     * ftfx_common_bit_operation() instead of using pre-processed MACROs or a
     * global variable in ftfx_common_bit_operation() to make sure that
     * ftfx_common_bit_operation() will be compiled into position-independent
     * code (PIC). */
    if (!isPreProcess) {
#if FLASH_CACHE_IS_CONTROLLED_BY_MCM
...
status_t FTFx_CACHE_PflashSetPrefetchSpeculation(
...
status_t FTFx_CACHE_PflashGetPrefetchSpeculation(
    ftfx_prefetch_speculation_status_t *speculationStatus) {
    (void)memset(speculationStatus, 0,
                 sizeof(ftfx_prefetch_speculation_status_t));
...
static void
ftfx_copy_common_bit_operation_to_ram(uint32_t *ftfxCommonBitOperation) {
    assert(sizeof(s_ftfxCommonBitOperationFunctionCode) <=
           ((uint32_t)kFTFx_CACHE_RamFuncMaxSizeInWords * 4U));
...
static void ftfx_common_bit_operation_command_sequence(
    ...
    *ftfxCommonBitOperationAddr += 1UL;
    callftfxCommonBitOperation_t ftfxCommonBitOperationCommand =
        config->bitOperFuncAddr.callFlashCommand;
    /* Workround for some devices which doesn't need this function */
    ftfxCommonBitOperationCommand((FTFx_REG32_ACCESS_TYPE)base, bitMask,
                                  bitShift, bitValue);
    *ftfxCommonBitOperationAddr -= 1UL;
}
#endif /*FTFx_DRIVER_IS_FLASH_RESIDENT*/
...
void mcm_flash_cache_clear(ftfx_cache_config_t *config) {
    ...
    ftfx_common_bit_operation_command_sequence(
        config, regBase, MCM_CACHE_CLEAR_MASK, MCM_CACHE_CLEAR_SHIFT, 1UL);
 ...
 * For device with secondary flash support, the OCMDR[1] is used to cache
 * secondary Pflash. */
#if FTFx_DRIVER_IS_FLASH_RESIDENT
    ...
    case kFLASH_MemoryIndexSecondaryFlash:
        /* calling flash command sequence function to execute the command */
        ftfx_common_bit_operation_command_sequence(
            config, (FTFx_REG32_ACCESS_TYPE)&MSCM_OCMDR1_REG,
    ...
    default:
        /* calling flash command sequence function to execute the command */
        ftfx_common_bit_operation_command_sequence(
            config, (FTFx_REG32_ACCESS_TYPE)&MSCM_OCMDR0_REG,
            MSCM_CACHE_CLEAR_MASK, MSCM_CACHE_CLEAR_SHIFT, setValue);
        break;
    }
#else  /* !FTFx_DRIVER_IS_FLASH_RESIDENT */
    switch (config->flashMemoryIndex) {
    case kFLASH_MemoryIndexSecondaryFlash:
        MSCM_OCMDR1_REG = (MSCM_OCMDR1_REG & (~MSCM_CACHE_CLEAR_MASK)) |
                          MSCM_CACHE_CLEAR(setValue);
    ...
    case kFLASH_MemoryIndexPrimaryFlash:
    default:
        MSCM_OCMDR0_REG = (MSCM_OCMDR0_REG & (~MSCM_CACHE_CLEAR_MASK)) |
                          MSCM_CACHE_CLEAR(setValue);
        /* Memory barriers for good measure.
...
void fmc_flash_cache_clear(ftfx_cache_config_t *config) {
    ...
    /* calling flash command sequence function to execute the command */
    ftfx_common_bit_operation_command_sequence(
        config, (FTFx_REG32_ACCESS_TYPE)&FMC_CACHE_REG, FMC_CACHE_CLEAR_MASK,
        FMC_CACHE_CLEAR_SHIFT, 0xFUL);
#else  /* !FTFx_DRIVER_IS_FLASH_RESIDENT */
    FMC_CACHE_REG =
        (FMC_CACHE_REG & (~FMC_CACHE_CLEAR_MASK)) | FMC_CACHE_CLEAR(~0);
    /* Memory barriers for good measure.
     * All Cache, Branch predictor and TLB maintenance operations before this
     * instruction complete */
    __ISB();
...
#if FLASH_PREFETCH_SPECULATION_IS_CONTROLLED_BY_MSCM
/*! @brief Performs the prefetch speculation buffer clear to the flash by
 * MSCM.*/
void mscm_flash_prefetch_speculation_enable(ftfx_cache_config_t *config,
                                            bool enable) {
    uint8_t setValue;
 ...
 * For device with secondary flash support, the OCMDR[1] is used to prefetch
 * secondary Pflash. */
#if FTFx_DRIVER_IS_FLASH_RESIDENT
    ...
    switch (config->flashMemoryIndex) {
    case kFLASH_MemoryIndexSecondaryFlash:
        MSCM_OCMDR1_REG = (MSCM_OCMDR1_REG & (~MSCM_SPECULATION_SET_MASK)) |
                          MSCM_SPECULATION_SET(setValue);
        /* Each cache clear instruction should be followed by below code*/
        __ISB();
        __DSB();
        break;
    case kFLASH_MemoryIndexPrimaryFlash:
    default:
        MSCM_OCMDR0_REG = (MSCM_OCMDR0_REG & (~MSCM_SPECULATION_SET_MASK)) |
                          MSCM_SPECULATION_SET(setValue);
        /* Memory barriers for good measure.
         * All Cache, Branch predictor and TLB maintenance operations before
         * this instruction complete */
...
void fmc_flash_prefetch_speculation_clear(ftfx_cache_config_t *config) {
    ...
    /* calling flash command sequence function to execute the command */
    ftfx_common_bit_operation_command_sequence(
        config, (FTFx_REG32_ACCESS_TYPE)&FMC_SPECULATION_INVALIDATE_REG,
