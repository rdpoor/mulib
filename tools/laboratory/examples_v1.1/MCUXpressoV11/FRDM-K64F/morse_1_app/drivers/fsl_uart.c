 ...
 * bus, so before disable TX, check kUART_TransmissionCompleteFlag to ensure the
 * TX is finished.
 *
 * @param base UART peripheral base address.
 ...
 */
static void UART_WriteNonBlocking(UART_Type *base, const uint8_t *data,
                                  size_t length);
...
static bool UART_TransferIsRxRingBufferFull(uart_handle_t *handle) {
    ...
    if (UART_TransferGetRxRingBufferLength(handle) ==
 ...
 * below shows how to use this API to configure UART. code uart_config_t
 * uartConfig; uartConfig.baudRate_Bps = 115200U; uartConfig.parityMode =
 * kUART_ParityDisabled; uartConfig.stopBitCount = kUART_OneStopBit;
 ...
 * clock source. retval kStatus_Success Status UART initialize succeed
 */
status_t UART_Init(UART_Type *base, const uart_config_t *config,
    ...
    assert((uint8_t)FSL_FEATURE_UART_FIFO_SIZEn(base) >=
    ...
    /* set sbrTemp to 1 if the sourceClockInHz can not satisfy the desired baud
     * rate */
    if (sbr == 0U) {
        sbr = 1U;
    }
#if defined(FSL_FEATURE_UART_HAS_BAUD_RATE_FINE_ADJUST_SUPPORT) &&             \
    FSL_FEATURE_UART_HAS_BAUD_RATE_FINE_ADJUST_SUPPORT
    ...
    baudDiff = (tempBaud > config->baudRate_Bps)
                   ? (tempBaud - config->baudRate_Bps)
                   : (config->baudRate_Bps - tempBaud);
    ...
    if (baudDiff >
        (config->baudRate_Bps - (srcClock_Hz / (16U * ((uint32_t)sbr + 1U))))) {
        baudDiff =
            config->baudRate_Bps - (srcClock_Hz / (16U * ((uint32_t)sbr + 1U)));
        sbr++;
    }
    ...
    base->C4 = (base->C4 & ~(uint8_t)UART_C4_BRFA_MASK) |
               ((uint8_t)brfa & UART_C4_BRFA_MASK);
#endif

    /* Set bit count/parity mode/idle type. */
    temp = base->C1 & ~((uint8_t)UART_C1_PE_MASK | (uint8_t)UART_C1_PT_MASK |
                        (uint8_t)UART_C1_M_MASK | (uint8_t)UART_C1_ILT_MASK);
    ...
    base->BDH = (base->BDH & ~(uint8_t)UART_BDH_SBNS_MASK) |
                (uint8_t)UART_BDH_SBNS((uint8_t)config->stopBitCount);
#endif
 ...
 * UART_Init. code UART_SetBaudRate(UART1, 115200U, 20000000U); endcode
 *
 * param base UART peripheral base address.
 * param baudRate_Bps UART baudrate to be set.
 * param srcClock_Hz UART clock source frequency in Hz.
 * retval kStatus_UART_BaudrateNotSupport Baudrate is not support in the current
 * clock source. retval kStatus_Success Set baudrate succeeded.
 */
status_t UART_SetBaudRate(UART_Type *base, uint32_t baudRate_Bps,
    ...
    /* Calculate the baud rate based on the temporary SBR values and BRFA */
    tempBaud = (srcClock_Hz * 2U / ((sbr * 32U + brfa)));
    baudDiff = (tempBaud > baudRate_Bps) ? (tempBaud - baudRate_Bps)
                                         : (baudRate_Bps - tempBaud);
#else
    /* Calculate the baud rate based on the temporary SBR values */
    ...
    if (baudDiff < ((baudRate_Bps / 100U) * 3U)) {
        ...
        /* Write the sbr value to the BDH and BDL registers*/
        base->BDH =
            (base->BDH & ~(uint8_t)UART_BDH_SBR_MASK) | (uint8_t)(sbr >> 8);
        base->BDL = (uint8_t)sbr;
    ...
    FSL_FEATURE_UART_HAS_BAUD_RATE_FINE_ADJUST_SUPPORT
        /* Write the brfa value to the register*/
        base->C4 = (base->C4 & ~(uint8_t)UART_C4_BRFA_MASK) |
                   ((uint8_t)brfa & (uint8_t)UART_C4_BRFA_MASK);
#endif
        /* Restore C2. */
        base->C2 = oldCtrl;
...
void UART_Enable9bitMode(UART_Type *base, bool enable) {
    ...
    if (enable) {
        /* Set UART_C1_M for 9-bit mode, clear UART_C1_PT and UART_C1_PE to
         * disable parity. */
        temp = base->C1 & ~((uint8_t)UART_C1_PE_MASK |
                            (uint8_t)UART_C1_PT_MASK | (uint8_t)UART_C1_M_MASK);
 ...
 * kUART_RxDataRegFullInterruptEnable); endcode
 *
 * param base UART peripheral base address.
...
void UART_EnableInterrupts(UART_Type *base, uint32_t mask) {
    ...
    /* The interrupt mask is combined by control bits from several register:
     * ((CFIFO<<24) | (C3<<16) | (C2<<8) |(BDH))
     */
 ...
 * full interrupt do the following. code
 ...
 * the return value with enumerators in ref _uart_interrupt_enable. For example,
 * to check whether TX empty interrupt is enabled, do the following. code
 ...
 * logical OR value of the enumerators ref _uart_flags. To check a specific
 * status, compare the return value with enumerators in ref _uart_flags. For
 * example, to check whether the TX is empty, do the following. code if
 * (kUART_TxDataRegEmptyFlag & UART_GetStatusFlags(UART1))
 *     {
 *         ...
 *     }
 * endcode
 ...
 *
 * param base UART peripheral base address.
 * param mask The status flags to be cleared; it is logical OR value of ref
 * _uart_flags. retval kStatus_UART_FlagCannotClearManually The flag can't be
 * cleared by this function but it is cleared automatically by hardware. retval
...
status_t UART_ClearStatusFlags(UART_Type *base, uint32_t mask) {
    ...
    if ((mask &
         ((uint32_t)kUART_IdleLineFlag | (uint32_t)kUART_NoiseErrorFlag |
          (uint32_t)kUART_FramingErrorFlag | (uint32_t)kUART_ParityErrorFlag |
          (uint32_t)kUART_RxOverrunFlag)) != 0u) {
        /* Read base->S1 and base->D to clear the flags. */
        (void)base->S1;
        (void)base->D;
#if defined(FSL_FEATURE_UART_HAS_FIFO) && FSL_FEATURE_UART_HAS_FIFO
        /* Read base->D may cause receiver underflow when there are no valid
           data. Clear receiver underflow flag */
    ...
    if ((mask & UART_GetStatusFlags(base)) != 0U) {
        ...
        are: kUART_TxDataRegEmptyFlag, kUART_TransmissionCompleteFlag,
        kUART_RxDataRegFullFlag, kUART_RxActiveFlag,
        kUART_NoiseErrorInRxDataRegFlag, kUART_ParityErrorInRxDataRegFlag,
        kUART_TxFifoEmptyFlag, kUART_RxFifoEmptyFlag. */
 ...
 * receiving data. retval kStatus_UART_NoiseError A noise error occurred while
 * receiving data. retval kStatus_UART_FramingError A framing error occurred
 * while receiving data. retval kStatus_UART_ParityError A parity error occurred
 * while receiving data. retval kStatus_UART_Timeout Transmission timed out and
 * was aborted. retval kStatus_Success Successfully received all data.
 */
    ...
    while (length-- != 0U) {
...
#endif
        {
            ...
            if (0U != (statusFlag & (uint32_t)kUART_RxOverrunFlag)) {
                status =
                    ((kStatus_Success ==
                      UART_ClearStatusFlags(base,
                                            (uint32_t)kUART_RxOverrunFlag))
                         ? (status_t)(kStatus_UART_RxHardwareOverrun)
                         : (status_t)(kStatus_UART_FlagCannotClearManually));
                /* If the OR bit is set all the other error flags are prevented
                   from setting, no need to check other status flags. */
                break;
            }
            ...
            if (0U != (statusFlag & (uint32_t)kUART_ParityErrorFlag)) {
                status =
                    ((kStatus_Success ==
                      UART_ClearStatusFlags(base,
                                            (uint32_t)kUART_ParityErrorFlag))
            ...
            if (0U != (statusFlag & (uint32_t)kUART_FramingErrorFlag)) {
                status =
                    ((kStatus_Success ==
                      UART_ClearStatusFlags(base,
                                            (uint32_t)kUART_FramingErrorFlag))
...
static void UART_ReadNonBlocking(UART_Type *base, uint8_t *data,
                                 size_t length) {
    assert(data);
 ...
 * other words, if p ringBufferSize is 32, only 31 bytes are used for saving
 * data.
 *
 * param base UART peripheral base address.
 * param handle UART handle pointer.
...
void UART_TransferStartRingBuffer(UART_Type *base, uart_handle_t *handle,
    ...
    UART_EnableInterrupts(base,
                          (uint32_t)kUART_RxDataRegFullInterruptEnable |
 ...
 * retval kStatus_UART_TxBusy Previous transmission still not finished; data not
 * all written to TX register yet. retval kStatus_InvalidArgument Invalid
 * argument.
 */
...
void UART_TransferAbortSend(UART_Type *base, uart_handle_t *handle) {
    ...
    UART_DisableInterrupts(
        base, (uint32_t)kUART_TxDataRegEmptyInterruptEnable |
                  (uint32_t)kUART_TransmissionCompleteInterruptEnable);
        ...
        if (handle->rxRingBuffer != NULL) {
            ...
            if (0U == bytesToReceive) {
                if (handle->callback != NULL) {
                    handle->callback(base, handle, kStatus_UART_RxIdle,
                                     handle->userData);
                }
            }
        }
        /* Ring buffer not used. */
            ...
            UART_EnableInterrupts(
                base, (uint32_t)kUART_RxDataRegFullInterruptEnable |
                          (uint32_t)kUART_RxOverrunInterruptEnable |
                          (uint32_t)kUART_FramingErrorInterruptEnable |
                          (uint32_t)kUART_IdleLineInterruptEnable);
            /* Enable parity error interrupt when parity mode is enable*/
            if ((UART_C1_PE_MASK & base->C1) != 0U) {
                UART_EnableInterrupts(
                    base, (uint32_t)kUART_ParityErrorInterruptEnable);
            }
        }
...
void UART_TransferHandleIRQ(UART_Type *base, uart_handle_t *handle) {
    ...
    if (((uint32_t)kUART_FramingErrorFlag & status) != 0U) {
        /* Read base->D to clear framing error flag, otherwise the RX does not
         * work. */
        while ((base->S1 & UART_S1_RDRF_MASK) != 0U) {
            (void)base->D;
        }
        ...
        /* Trigger callback. */
        if (handle->callback != NULL) {
            handle->callback(base, handle, kStatus_UART_FramingError,
                             handle->userData);
        }
    }
    ...
    if (((uint32_t)kUART_ParityErrorFlag & status) != 0U) {
        /* Read base->D to clear parity error flag, otherwise the RX does not
         * work. */
        while ((base->S1 & UART_S1_RDRF_MASK) != 0U) {
            (void)base->D;
        }
        ...
        /* Trigger callback. */
        if (handle->callback != NULL) {
            handle->callback(base, handle, kStatus_UART_ParityError,
                             handle->userData);
        }
    }
    ...
    if (((uint32_t)kUART_RxOverrunFlag & status) != 0U) {
        ...
        /* Trigger callback. */
        if (handle->callback != NULL) {
            handle->callback(base, handle, kStatus_UART_RxHardwareOverrun,
                             handle->userData);
        }
    }

    /* If IDLE line was detected. */
    if ((((uint32_t)kUART_IdleLineFlag & status) != 0U) &&
        ((UART_C2_ILIE_MASK & base->C2) != 0U)) {
#if defined(FSL_FEATURE_UART_HAS_FIFO) && FSL_FEATURE_UART_HAS_FIFO
        /* If still some data in the FIFO, read out these data to user data
         * buffer. */
        count = base->RCFIFO;
        ...
        while ((count != 0U) && (handle->rxDataSize != 0U)) {
            ...
            if (0U == handle->rxDataSize) {
                ...
                /* Disable RX interrupt/overrun interrupt/fram error/idle line
                 * detected interrupt */
                UART_DisableInterrupts(
                    base, (uint32_t)kUART_RxDataRegFullInterruptEnable |
                              (uint32_t)kUART_RxOverrunInterruptEnable |
        ...
        if ((handle->callback != NULL) && (handle->rxDataSize != 0U)) {
            handle->callback(base, handle, kStatus_UART_IdleLineDetected,
                             handle->userData);
        }
    }
    /* Receive data register full */
    if ((((uint32_t)kUART_RxDataRegFullFlag & status) != 0U) &&
        ((UART_C2_RIE_MASK & base->C2) != 0U)) {
/* Get the size that can be stored into buffer for this interrupt. */
    ...
    /* If framing error or parity error happened, stop the RX interrupt when use
     * no ring buffer */
    if (((handle->rxState == (uint8_t)kUART_RxFramingError) ||
         (handle->rxState == (uint8_t)kUART_RxParityError)) &&
    ...
    if ((((uint32_t)kUART_TxDataRegEmptyFlag & status) != 0U) &&
        ((base->C2 & UART_C2_TIE_MASK) != 0U)) {
/* Get the bytes that available at this moment. */
    ...
    if ((0U != ((uint32_t)kUART_TransmissionCompleteFlag & status)) &&
        (0U != (base->C2 & UART_C2_TCIE_MASK))) {
        ...
        /* Trigger callback. */
        if (handle->callback != NULL) {
            handle->callback(base, handle, kStatus_UART_TxIdle,
                             handle->userData);
        }
    }
}
...
#if defined(FSL_FEATURE_UART_HAS_SHARED_IRQ0_IRQ1_IRQ2_IRQ3) &&                \
    FSL_FEATURE_UART_HAS_SHARED_IRQ0_IRQ1_IRQ2_IRQ3
...
void UART0_UART1_UART2_UART3_DriverIRQHandler(void) {
    for (uint32_t instance = 0U; instance < 4U; instance++) {
     ...
     ((defined(FSL_FEATURE_SOC_LPSCI_COUNT)) &&                                \
