...
#if defined(FSL_FEATURE_FLASH_HAS_SWAP_CONTROL_CMD) &&                         \
    FSL_FEATURE_FLASH_HAS_SWAP_CONTROL_CMD
/*!
...
/*! @brief init access segment feature */
    ...
    FSL_FEATURE_FLASH_HAS_ACCESS_CONTROL
...
static uint32_t flash_calculate_prot_segment_size(uint32_t flashSize,
...
/*! @brief Decide whether to convert the start address from primary flash to
 * secondary flash based on the current start address*/
static void flash_convert_start_address(ftfx_config_t *config, uint32_t start);

#if defined(FSL_FEATURE_FLASH_HAS_PFLASH_BLOCK_SWAP) &&                        \
    FSL_FEATURE_FLASH_HAS_PFLASH_BLOCK_SWAP
/*! @brief Validates the given address to see if it is equal to swap indicator
 * address in pflash swap IFR.*/
static status_t flash_validate_swap_indicator_address(ftfx_config_t *config,
                                                      uint32_t address);
...
static volatile uint32_t *const kFPROTL =
    (volatile uint32_t *)(uint32_t)&FTFx_FPROT_LOW_REG;
 ...
 *
 *  The values in this table have been right shifted 10 bits so that they will
 * all fit within an 16-bit integer. To get the actual flash density, you must
 * left shift the looked up value by 10 bits.
 *
 *  Elements of this table have a value of 0 in cases where the PFSIZE bitfield
 * value is reserved.
 *
 *  Code to use the table:
 *  @code
 *      uint8_t pfsize = (SIM->FCFG1 & SIM_FCFG1_PFSIZE_MASK) >>
 * SIM_FCFG1_PFSIZE_SHIFT; flashDensity = ((uint32_t)kPFlashDensities[pfsize])
 * << 10;
 *  @endcode
 */
#if defined(FSL_FEATURE_FLASH_SIZE_ENCODING_RULE_VERSION) &&                   \
    (FSL_FEATURE_FLASH_SIZE_ENCODING_RULE_VERSION == 1)
 ...
 * @retval #kStatus_FTFx_ExecuteInRamFunctionNotReady Execute-in-RAM function is
 * not available.
 * @retval #kStatus_FTFx_PartitionStatusUpdateFailure Failed to update the
 * partition status.
 */
status_t FLASH_Init(flash_config_t *config) {
    ...
    for (uint8_t flashIndex = 0U; flashIndex < FTFx_FLASH_COUNT; flashIndex++) {
        /* init flash type, kinetis has Pflash and flxnvm, pflash is often used
         * to store executable code and flexnvm can be used as date flash to
         * store user data, and can also be configured as eeprom backup space
         * with flexram.
         */
        config->ftfxConfig[flashIndex].flashDesc.type =
            (uint8_t)kFTFx_MemTypePflash;
    ...
    FSL_FEATURE_FLASH_HAS_ACCESS_CONTROL
        if (0U !=
            (config->ftfxConfig[flashIndex].flashDesc.feature.hasXaccControl)) {
        ...
        if (0U !=
            (config->ftfxConfig[flashIndex].flashDesc.feature.hasProtControl))
#endif
        {
...
/*!
 * @brief Erases the Dflash sectors encompassed by parameters passed into
 * function.
 *
 ...
 *
 * @retval #kStatus_FTFx_Success API was executed successfully; the appropriate
 * number of flash sectors based on the desired start address and length was
 * erased successfully.
 *
 * @retval #kStatus_FTFx_InvalidArgument An invalid argument is provided.
 * @retval #kStatus_FTFx_AlignmentError The parameter is not aligned with the
 * specified baseline.
 ...
 * @retval #kStatus_FTFx_ExecuteInRamFunctionNotReady Execute-in-RAM function is
 * not available.
 * @retval #kStatus_FTFx_AccessError Invalid instruction codes and out-of bounds
 * addresses.
 * @retval #kStatus_FTFx_ProtectionViolation The program/erase operation is
 * requested to execute on protected areas.
 * @retval #kStatus_FTFx_CommandFailure Run-time error during the command
 * execution.
 */
status_t FLASH_Erase(flash_config_t *config, uint32_t start,
    ...
    returnCode = flash_check_range_to_get_index(config, start, lengthInBytes,
                                                &flashIndex);
    if (returnCode != kStatus_FTFx_Success) {
        return returnCode;
    }

    /* Decide whether to convert the start address from primary flash to
     * secondary flash based on the current address */
    flash_convert_start_address(&config->ftfxConfig[flashIndex], start);

    return FTFx_CMD_Erase(&config->ftfxConfig[flashIndex], start, lengthInBytes,
                          key);
}
 ...
 * function.
 *
 * This function erases one flash sector size based on the start address, and it
 * is executed asynchronously.
 */
status_t FLASH_EraseSectorNonBlocking(flash_config_t *config, uint32_t start,
                                      uint32_t key) {
    status_t returnCode;
    uint8_t flashIndex;
    ...
    return FTFx_CMD_EraseSectorNonBlocking(&config->ftfxConfig[flashIndex],
                                           start, key);
}
...
status_t FLASH_Program(flash_config_t *config, uint32_t start, uint8_t *src,
    ...
    /* convert the start address from primary flash to secondary flash based on
     * the current address */
    flash_convert_start_address(&config->ftfxConfig[flashIndex], start);

    /* Programs flash */
    return FTFx_CMD_Program(&config->ftfxConfig[flashIndex], start, src,
                            lengthInBytes);
}
 ...
 */
status_t FLASH_ProgramOnce(flash_config_t *config, uint32_t index, uint8_t *src,
                           uint32_t lengthInBytes) {
    return FTFx_CMD_ProgramOnce(&config->ftfxConfig[0], index, src,
                                lengthInBytes);
}
...
/*!
 * @brief Programs flash with data at locations passed in through parameters via
 * the Program Section command.
 *
 * This function programs the flash memory with the desired data for a given
...
status_t FLASH_ProgramSection(flash_config_t *config, uint32_t start,
    ...
    return FTFx_CMD_ProgramSection(&config->ftfxConfig[flashIndex], start, src,
                                   lengthInBytes);
}
#endif

#if defined(FSL_FEATURE_FLASH_HAS_READ_RESOURCE_CMD) &&                        \
    FSL_FEATURE_FLASH_HAS_READ_RESOURCE_CMD
/*!
 * @brief Reads the resource with data at locations passed in through
 * parameters.
 */
status_t FLASH_ReadResource(flash_config_t *config, uint32_t start,
                            uint8_t *dst, uint32_t lengthInBytes,
                            ftfx_read_resource_opt_t option) {
    return FTFx_CMD_ReadResource(&config->ftfxConfig[0], start, dst,
                                 lengthInBytes, option);
}
#endif
 ...
 * @brief Reads the Program Once Field through parameters.
 */
status_t FLASH_ReadOnce(flash_config_t *config, uint32_t index, uint8_t *dst,
                        uint32_t lengthInBytes) {
...
status_t FLASH_VerifyErase(flash_config_t *config, uint32_t start,
    ...
    return FTFx_CMD_VerifyErase(&config->ftfxConfig[flashIndex], start,
                                lengthInBytes, margin);
}
...
/*!
 * @brief Verifies programming of the desired flash area at a specified margin
 * level.
 *
...
status_t FLASH_VerifyProgram(flash_config_t *config, uint32_t start,
    ...
    return FTFx_CMD_VerifyProgram(&config->ftfxConfig[flashIndex], start,
                                  lengthInBytes, expectedData, margin,
...
#if defined(FSL_FEATURE_FLASH_HAS_SET_FLEXRAM_FUNCTION_CMD) &&                 \
 ...
 */
status_t FLASH_Swap(flash_config_t *config, uint32_t address,
                    bool isSetEnable) {
    status_t returnCode;
    ...
    returnCode =
        flash_check_range_to_get_index(config, address, 1U, &flashIndex);
    if (returnCode != kStatus_FTFx_Success) {
        return returnCode;
    }
    ...
    do {
        returnCode = FTFx_CMD_SwapControl(ftfxConfig, address,
        ...
        if (!isSetEnable) {
            if (returnInfo.flashSwapState == kFTFx_SwapStateDisabled) {
                return kStatus_FTFx_Success;
            } else if (returnInfo.flashSwapState ==
                       kFTFx_SwapStateUninitialized) {
                /* The swap system changed to the DISABLED state with Program
                 * flash block 0 located at relative flash address 0x0_0000 */
                returnCode = FTFx_CMD_SwapControl(
                    ftfxConfig, address, kFTFx_SwapControlOptionDisableSystem,
                    &returnInfo);
            } else {
                /* Swap disable should be requested only when swap system is in
                 * the uninitialized state */
                return kStatus_FTFx_SwapSystemNotInUninitialized;
            }
        } else {
            /* When first swap: the initial swap state is Uninitialized, flash
             * swap indicator address is unset, the swap procedure should be
             * Uninitialized -> Update-Erased -> Complete. After the first swap
             * has been completed, the flash swap inidicator address cannot be
             * modified unless EraseAllBlocks command is issued, the swap
             * procedure is changed to Update -> Update-Erased -> Complete. */
            switch (returnInfo.flashSwapState) {
            case kFTFx_SwapStateUninitialized:
                /* If current swap mode is Uninitialized, Initialize Swap to
                 * Initialized/READY state. */
                returnCode = FTFx_CMD_SwapControl(
                    ftfxConfig, address, kFTFx_SwapControlOptionIntializeSystem,
                    &returnInfo);
                break;
            case kFTFx_SwapStateReady:
                /* Validate whether the address provided to the swap system is
                 * matched to swap indicator address in the IFR */
                returnCode =
                    flash_validate_swap_indicator_address(ftfxConfig, address);
                if (returnCode == kStatus_FTFx_Success) {
                    /* If current swap mode is Initialized/Ready, Initialize
                     * Swap to UPDATE state. */
                    returnCode = FTFx_CMD_SwapControl(
                        ftfxConfig, address,
                        kFTFx_SwapControlOptionSetInUpdateState, &returnInfo);
                }
                break;
            case kFTFx_SwapStateUpdate:
                /* If current swap mode is Update, Erase indicator sector in non
                 * active block to proceed swap system to update-erased state */
                returnCode = FLASH_Erase(
                    config, address + (ftfxConfig->flashDesc.totalSize >> 1u),
                    ftfxConfig->opsConfig.addrAligment.sectorCmd,
                    (uint32_t)kFTFx_ApiEraseKey);
                break;
            case kFTFx_SwapStateUpdateErased:
                /* If current swap mode is Update or Update-Erased, progress
                 * Swap to COMPLETE State */
                returnCode = FTFx_CMD_SwapControl(
                    ftfxConfig, address,
                    kFTFx_SwapControlOptionSetInCompleteState, &returnInfo);
                break;
            case kFTFx_SwapStateComplete:
                break;
            case kFTFx_SwapStateDisabled:
                /* When swap system is in disabled state, We need to clear swap
                 * system back to uninitialized by issuing EraseAllBlocks
                 * command */
    ...
    } while (!((kFTFx_SwapStateComplete == returnInfo.flashSwapState) &&
               isSetEnable));
...
status_t FLASH_IsProtected(flash_config_t *config, uint32_t start,
...
#endif
    {
        ...
        uint32_t regionCheckedCounter; /* increments each time the flash address
                                        * was checked for protection status */
        uint32_t regionCounter; /* incrementing variable used to increment
                                 * through the flash protection regions */
        uint32_t protectStatusCounter; /* increments each time a flash region
                                          was detected as protected */
        uint8_t flashRegionProtectStatus
            [MAX_FLASH_PROT_REGION_COUNT]; /* array of the protection
                                            * status for each
                                            * protection region */
        ...
        {         /* unprotected state by default. */
            flashRegionProtectStatus[i] =
                (uint8_t)0xFF; /* The array is initialized to all 1 */
        }

        uint32_t flashRegionAddress
            [MAX_FLASH_PROT_REGION_COUNT +
             1U]; /* array of the start addresses for each flash
        ...
        /* populate the flashRegionAddress array with the start address of each
         * flash region */
        regionCounter =
            0U; /* make sure regionCounter is initialized to 0 first */
        /* populate up to 33rd element of array, this is the next address after
         * end of flash array */
         ...
         * Protection status for each region is stored in the FPROT[3:0]
         * registers Each bit represents one region of flash 4 registers *
         * 8-bits-per-register = 32-bits (32-regions) The convention is:
         ...
         * regionCounter is used to determine which FPROT[3:0] register to check
         * for protection status Note: FPROT=1 means NOT protected, FPROT=0
         * means protected */
        regionCounter =
            0U; /* make sure regionCounter is initialized to 0 first */
        static volatile uint32_t *const kFPROTLx =
            (volatile uint32_t *)(uint32_t)&FTFx_FPROTL3_REG;

#if defined(FTFx_FLASH0_HAS_HIGH_PROT_REG) && FTFx_FLASH0_HAS_HIGH_PROT_REG
        static volatile uint32_t *const kFPROTHx =
            (volatile uint32_t *)(uint32_t)&FTFx_FPROTH3_REG;
#endif

#if defined(FTFx_FLASH1_HAS_INT_PROT_REG) && FTFx_FLASH1_HAS_INT_PROT_REG
        static volatile uint16_t *const kFPROTSx =
            (volatile uint16_t *)(uint32_t)&FTFx_FPROTSL_REG;
#endif
        while (regionCounter < ftfxConfig->flashDesc.protectRegionMem.count) {
#if (FTFx_FLASH_COUNT > 1U)
            if ((0U == ftfxConfig->flashDesc.index) ||
                (0U != ftfxConfig->flashDesc.feature.hasIndProtReg))
#endif
            {
                ...
                if (regionCounter < (uint32_t)MAX_FLASH_PROT_REGION_COUNT) {
                    flashRegionProtectStatus[regionCounter] =
                        (uint8_t)(((kFPROTLx[0]) >> regionCounter) & 0x1U);
                }
#else
                if (regionCounter < 32u) {
                    ...
                    flashRegionProtectStatus[regionCounter] = (uint8_t)(
                        ((kFPROTHx[0]) >> (regionCounter - 32U)) & 0x1U);
                }
#endif
                else {
                    isBreakNeeded = true;
...
#if defined(FTFx_FLASH1_HAS_INT_PROT_REG) && FTFx_FLASH1_HAS_INT_PROT_REG
            else if ((1U == ftfxConfig->flashDesc.index) &&
                     (0U != ftfxConfig->flashDesc.feature.hasIndProtReg)) {
                /* Note: So far protection region count may be 8/16 */
                if (regionCounter < 16U) {
                    flashRegionProtectStatus[regionCounter] =
                        (uint8_t)((kFPROTSx[0] >> regionCounter) & (0x01u));
                } else {
                    isBreakNeeded = true;
                }
                regionCounter++;
        ...
        protectStatusCounter =
            0U; /* make sure protectStatusCounter is initialized to 0 first */
        while (start < endAddress) {
            /* check to see if the address falls within this protection region
             * Note that if the entire flash is to be checked, the last
             * protection region checked would consist of the last protection
             * start address and the start address following the end of flash */
            if ((start >= flashRegionAddress[regionCounter]) &&
                (start < flashRegionAddress[regionCounter + 1U])) {
                /* increment regionCheckedCounter to indicate this region was
                 * checked */
                ...
                if (0U == flashRegionProtectStatus[regionCounter]) {
                    /* increment protectStatusCounter to indicate this region is
                     * protected */
                    protectStatusCounter++;
                }
                start += ftfxConfig->flashDesc.protectRegionMem
                             .size; /* increment to an address within the next
                                       region */
            }
            regionCounter++; /* increment regionCounter to check for the next
                                flash protection region */
        }

        /* if protectStatusCounter == 0, then no region of the desired flash
         * region is protected */
        if (protectStatusCounter == 0U) {
            *protection_state = kFLASH_ProtectionStateUnprotected;
        }
        /* if protectStatusCounter == regionCheckedCounter, then each region
           checked was protected */
        else if (protectStatusCounter == regionCheckedCounter) {
            *protection_state = kFLASH_ProtectionStateProtected;
        }
        /* if protectStatusCounter != regionCheckedCounter, then protection
         * status is mixed In other words, some regions are protected while
    ...
    FSL_FEATURE_FLASH_HAS_ACCESS_CONTROL
/*!
 * @brief Returns the access state of the desired flash area via the pointer
 * passed into the function.
 *
...
status_t FLASH_IsExecuteOnly(flash_config_t *config, uint32_t start,
    ...
    uint32_t alignedStartAddress =
        ALIGN_DOWN(start, ftfxConfig->flashDesc.accessSegmentMem.size);
    uint32_t alignedEndAddress =
        ALIGN_UP(endAddress, ftfxConfig->flashDesc.accessSegmentMem.size);
    ...
    uint32_t maxSupportedExecuteOnlySegmentCount =
        ...
        ftfxConfig->flashDesc.accessSegmentMem.size;
    ...
    while (start < endAddress) {
        ...
        segmentIndex = (start - ftfxConfig->flashDesc.accessSegmentMem.base) /
                       ftfxConfig->flashDesc.accessSegmentMem.size;

        if ((0U == ftfxConfig->flashDesc.index) ||
            (0U != ftfxConfig->flashDesc.feature.hasIndXaccReg)) {
            /* For primary flash, The eight XACC registers allow up to 64
             * restricted segments of equal memory size.
             */
            if (segmentIndex < 32U) {
                xacc = *(const volatile uint32_t *)(uint32_t)&FTFx_XACCL3_REG;
            } else if (segmentIndex <
                       ftfxConfig->flashDesc.accessSegmentMem.count) {
        ...
        else if ((ftfxConfig->flashDesc.index == 1U) &&
                 (0u != ftfxConfig->flashDesc.feature.hasIndXaccReg)) {
            /* For secondary flash, The two XACCS registers allow up to 16
             * restricted segments of equal memory size.
             */
            if (segmentIndex < 8U) {
        ...
        /* Determine if this address range is in a execute-only protection flash
         * segment. */
    ...
    } else if (executeOnlySegmentCounter <
               maxSupportedExecuteOnlySegmentCount) {
 ...
 * protection register. Each bit is corresponding to protection of 1/32(64) of
 * the total PFlash. The least significant bit is corresponding to the lowest
 * address area of PFlash. The most significant bit is corresponding to the
 * highest address area of PFlash. There are two possible cases as shown below:
...
status_t FLASH_PflashSetProtection(flash_config_t *config,
     ...
     * define which program flash regions are protected from program and erase
     * operations. Protected flash regions cannot have their content changed;
     * that is, these regions cannot be programmed and cannot be erased by any
     * flash command
     */
#if (FTFx_FLASH_COUNT > 1U)
...
#endif
    {
        if (config->ftfxConfig[0].flashDesc.feature.ProtRegBits >= 32U) {
            /* set PFlash protection register, unprotected regions are marked
             * with a 1 and setting PFlash protection register, unprotected
             * regions are marked with a 1 and protected regions use a 0; each
             * bit of FPROT register can only be changed from 1s to 0s while all
             * bits with 0s to 1s transitions are ignored.
             */
...
#if defined(FTFx_FLASH0_HAS_HIGH_PROT_REG) && FTFx_FLASH0_HAS_HIGH_PROT_REG
        /* For primary flash with eight PROT registers allow up to 64 protected
         * segments of equal memory size. */
        ...
        /* For secondary flash with two FPROT registers allow up to 16 protected
         * segments of equal memory size. */
 ...
 * @param protectStatus  Protect status returned by the PFlash IP. Each bit is
 * corresponding to the protection of 1/32(64) of the total PFlash. The least
 * significant bit corresponds to the lowest address area of the PFlash. The
 * most significant bit corresponds to the highest address area of PFlash. There
 * are two possible cases as shown below: 0: this area is protected. 1: this
 * area is unprotected.
 *
 * @retval #kStatus_FTFx_Success API was executed successfully.
 * @retval #kStatus_FTFx_InvalidArgument An invalid argument is provided.
...
status_t FLASH_GetProperty(flash_config_t *config,
    ...
    case kFLASH_PropertyPflash0BlockSize:
        *value = config->ftfxConfig[0].flashDesc.totalSize /
                 config->ftfxConfig[0].flashDesc.blockCount;
        break;
    ...
    case kFLASH_PropertyPflash0FacSupport:
        *value =
            (uint32_t)config->ftfxConfig[0].flashDesc.feature.hasXaccControl;
        break;
    ...
    case kFLASH_PropertyPflash1BlockSize:
        *value = config->ftfxConfig[1].flashDesc.totalSize /
                 config->ftfxConfig[1].flashDesc.blockCount;
        break;
    ...
    case kFLASH_PropertyPflash1FacSupport:
        *value =
            (uint32_t)config->ftfxConfig[1].flashDesc.feature.hasXaccControl;
        break;
...
static void flash_init_features(ftfx_config_t *config) {
    ...
    /* if another flash exists */
        ...
        config->flashDesc.feature.hasIndPfsizeReg =
            config->flashDesc.feature.isIndBlock;
    ...
    config->flashDesc.feature.hasXaccControl =
        FSL_FEATURE_FLASH_HAS_ACCESS_CONTROL;
}
...
static void flash_opsonfig_Init(flash_config_t *config, uint8_t flashIndex) {
    ...
    uint32_t pflashSectorCmdAlignment;  /* store P-Flash Erase sector command
                                           address alignment */
    uint32_t pflashSectionCmdAlignment; /* store Rrogram/Verify section command
                                           address alignment */
    ...
    if (flashIndex == 1U) {
        ...
        pflashSectorCmdAlignment =
            FLASH1_FEATURE_PFLASH_SECTOR_CMD_ADDRESS_ALIGMENT;
        pflashSectionCmdAlignment =
            FLASH1_FEATURE_PFLASH_SECTION_CMD_ADDRESS_ALIGMENT;
    ...
    {
        pflashStartAddress =
            FLASH0_FEATURE_PFLASH_START_ADDRESS; /* get P-Flash start address */
        ...
        pflashSectionCmdAlignment =
            FLASH0_FEATURE_PFLASH_SECTION_CMD_ADDRESS_ALIGMENT;
...
#endif
    {
        ...
        config->ftfxConfig[flashIndex].flashDesc.totalSize =
            flash_calculate_mem_size(pflashBlockCount, pflashBlockSize,
                                     pfsizeMask, pfsizeShift);
    }
#if (FTFx_FLASH_COUNT > 1U)
    else {
        config->ftfxConfig[flashIndex].flashDesc.totalSize =
            pflashBlockCount * pflashBlockSize;
    }
#endif

    /* init P-Flash write unit size */
    config->ftfxConfig[flashIndex].opsConfig.addrAligment.blockWriteUnitSize =
        (uint8_t)pflashBlockWriteUnitSize;
    /* init P-Flash Erase sector command address alignment */
    config->ftfxConfig[flashIndex].opsConfig.addrAligment.sectorCmd =
        (uint8_t)pflashSectorCmdAlignment;
    /* init P-Flash Rrogram/Verify section command address alignment */
    config->ftfxConfig[flashIndex].opsConfig.addrAligment.sectionCmd =
        (uint8_t)pflashSectionCmdAlignment;
...
static void flash_access_init(flash_config_t *config, uint8_t flashIndex) {
    ...
    if (flashIndex == 1U) {
        ...
        specMem->size = (uint32_t)kFTFx_AccessSegmentUnitSize
                        << FTFx_FACSSS_REG;
...
static void flash_protection_init(flash_config_t *config, uint8_t flashIndex) {
...
#endif // #if (FTFx_FLASH_COUNT > 1U)
    {
        /* store flash0 Protection region count */
        pflashProtectionRegionCount =
            FLASH0_FEATURE_PFLASH_PROTECTION_REGION_COUNT;
    }
...
#endif // #if (FTFx_FLASH_COUNT > 1U)
    {
        ...
        specMem->size = flash_calculate_prot_segment_size(
            config->ftfxConfig[flashIndex].flashDesc.totalSize, specMem->count);
    }
#if (FTFx_FLASH_COUNT > 1U)
    else {
        ...
        if (flashIndex == FTFx_FLASH_COUNT - 1U) {
            ...
            segmentSize = flash_calculate_prot_segment_size(pflashTotalSize,
                                                            specMem->count);
            for (i = 0U; i < FTFx_FLASH_COUNT; i++) {
                /* init flash0 and flash1 port segment size */
                config->ftfxConfig[i].flashDesc.protectRegionMem.size =
                    segmentSize;
            }
        }
    }
...
static uint32_t flash_calculate_mem_size(uint32_t pflashBlockCount,
    ...
    /* PFSIZE=0xf means that on customer parts the IFR was not correctly
     * programmed. We just use the pre-defined flash size in feature file here
     * to support pre-production parts */
...
static uint32_t flash_calculate_prot_segment_size(uint32_t flashSize,
     ...
     * If the flash density is > 32KB, then protection region is 1/32 of total
     * flash density Else if flash density is < 32KB, then flash protection
     * region is set to 1KB */
...
static status_t flash_check_range_to_get_index(flash_config_t *config,
    ...
    for (uint8_t index = 0U; index < FTFx_FLASH_COUNT; index++) {
        if ((start >= config->ftfxConfig[index].flashDesc.blockBase) &&
            ((start + lengthInBytes) <=
             (config->ftfxConfig[index].flashDesc.blockBase +
              config->ftfxConfig[index].flashDesc.totalSize))) {
...
#if (FTFx_FLASH_COUNT > 1U)
    if ((0U != config->flashDesc.index) &&
        (0U != config->flashDesc.feature.isIndBlock)) {
        /* When required by the command, address bit 23 selects between main
         * flash memory
         * (=0) and secondary flash memory (=1).*/
        config->opsConfig.convertedAddress =
            start - config->flashDesc.blockBase + 0x800000U;
    } else
#endif
    {
...
static status_t flash_validate_swap_indicator_address(ftfx_config_t *config,
...
#if defined(FSL_FEATURE_FLASH_HAS_READ_RESOURCE_CMD) &&                        \
    FSL_FEATURE_FLASH_HAS_READ_RESOURCE_CMD
    returnCode = FTFx_CMD_ReadResource(
        config, config->ifrDesc.resRange.pflashSwapIfrStart,
...
#else
    {
        ...
        returnCode =
            FTFx_CMD_ReadOnce(config, (uint32_t)kFLASH_RecordIndexSwapAddr,
                              (uint8_t *)returnValue, 4U);
        if (returnCode != kStatus_FTFx_Success) {
            return returnCode;
        }
        flashSwapIfrFieldData.swapIndicatorAddress = (uint16_t)returnValue[0];
        returnCode =
            FTFx_CMD_ReadOnce(config, (uint32_t)kFLASH_RecordIndexSwapEnable,
                              (uint8_t *)returnValue, 4U);
        if (returnCode != kStatus_FTFx_Success) {
            return returnCode;
        }
        ...
        returnCode =
            FTFx_CMD_ReadOnce(config, (uint32_t)kFLASH_RecordIndexSwapDisable,
                              (uint8_t *)returnValue, 4U);
        if (returnCode != kStatus_FTFx_Success) {
            return returnCode;
        }
    ...
    /* The high bits value of Swap Indicator Address is stored in Program Flash
     * Swap IFR Field, the low several bit value of Swap Indicator Address is
     * always 1'b0 */
    swapIndicatorAddress =
        (uint32_t)flashSwapIfrFieldData.swapIndicatorAddress *
...
status_t FLASH_GetCommandState(void) {
    ...
    idleFlag = ((uint32_t)registerValue & FTFx_FSTAT_CCIF_MASK) >>
               FTFx_FSTAT_CCIF_SHIFT;
    if (idleFlag == 0U) {
