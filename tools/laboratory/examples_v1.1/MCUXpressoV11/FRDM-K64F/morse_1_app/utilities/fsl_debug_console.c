...
typedef struct DebugConsoleState {
    ...
    hal_uart_status_t (*putChar)(
        hal_uart_handle_t handle, const uint8_t *data,
        size_t length); /*!< put char function pointer */
    hal_uart_status_t (*getChar)(
        hal_uart_handle_t handle, uint8_t *data,
...
static int DbgConsole_ScanfFormattedData(const char *line_ptr, char *format,
                                         va_list args_ptr);
...
status_t DbgConsole_Init(uint8_t instance, uint32_t baudRate,
    ...
    (void)HAL_UartInit((hal_uart_handle_t)&s_debugConsole.uartHandleBuffer[0],
                       &usrtConfig);
 ...
 */
static void DbgConsole_PrintfPaddingCharacter(char c, int32_t curlen,
 ...
 */
static int32_t DbgConsole_ConvertRadixNumToString(char *numstr, void *nump,
                                                  int32_t neg, int32_t radix,
                                                  bool use_caps) {
#if PRINTF_ADVANCED_ENABLE
    int64_t a;
    int64_t b;
 ...
 */
static int32_t
DbgConsole_ConvertFloatRadixNumToString(char *numstr, void *nump, int32_t radix,
...
static int DbgConsole_PrintfFormattedData(PUTCHAR_FUNC func_ptr,
    ...
    while (true) {
        ...
        while (!done) {
            ...
            if ((c >= '0') && (c <= '9')) {
                field_width =
                    (field_width * 10U) + ((uint32_t)c - (uint32_t)'0');
            }
#if PRINTF_ADVANCED_ENABLE
            else if (c == '*') {
        ...
        if (*++p == '.') {
            ...
            while (!done) {
                ...
                if ((c >= '0') && (c <= '9')) {
                    precision_width =
                        (precision_width * 10U) + ((uint32_t)c - (uint32_t)'0');
#if PRINTF_ADVANCED_ENABLE
        ...
        c = *++p;
        {
            if ((c == 'd') || (c == 'i') || (c == 'f') || (c == 'F') ||
                ...
                if ((c == 'd') || (c == 'i')) {
                    ...
                    vlen = DbgConsole_ConvertRadixNumToString(vstr, &ival, 1,
                                                              10, use_caps);
                    vstrp = &vstr[vlen];
#if PRINTF_ADVANCED_ENABLE
                    if (ival < 0) {
                    ...
                    if (0U != (flags_used & (uint32_t)kPRINTF_Zero)) {
                        ...
                        DbgConsole_PrintfPaddingCharacter(
                            '0', vlen, (int32_t)field_width, &count, func_ptr);
                        vlen = (int32_t)field_width;
                    } else {
                        if (0U == (flags_used & (uint32_t)kPRINTF_Minus)) {
                            DbgConsole_PrintfPaddingCharacter(
                                ' ', vlen, (int32_t)field_width, &count,
                                func_ptr);
                            if ('\0' != schar) {
                                (void)func_ptr(schar);
                                count++;
                ...
                if ((c == 'X') || (c == 'x')) {
                    ...
                    vlen = DbgConsole_ConvertRadixNumToString(vstr, &uval, 0,
                                                              16, use_caps);
                    vstrp = &vstr[vlen];
                ...
                if ((c == 'o') || (c == 'b') || (c == 'p') || (c == 'u')) {
                    ...
                    vlen = DbgConsole_ConvertRadixNumToString(
                        vstr, &uval, 0, (int32_t)radix, use_caps);
                    vstrp = &vstr[vlen];
#if PRINTF_ADVANCED_ENABLE
                    if (0U != (flags_used & (uint32_t)kPRINTF_Zero)) {
                        DbgConsole_PrintfPaddingCharacter(
...
#if !PRINTF_ADVANCED_ENABLE
                DbgConsole_PrintfPaddingCharacter(
                    ' ', vlen, (int32_t)field_width, &count, func_ptr);
#endif /* !PRINTF_ADVANCED_ENABLE */
                if (vstrp != NULL) {
                    while ('\0' != *vstrp) {
                        (void)func_ptr(*vstrp--);
...
size_t __write(int handle, const unsigned char *buffer, size_t size) {
    ...
    if (NULL == buffer) {
        ...
        /* Send data. */
        (void)s_debugConsole.putChar(
            (hal_uart_handle_t)&s_debugConsole.uartHandleBuffer[0], buffer,
            size);
        ret = size;
    }
    return ret;
...
int __attribute__((weak)) __sys_readc(void) {
    ...
    /* Receive data. */
    s_debugConsole.getChar(
        (hal_uart_handle_t)&s_debugConsole.uartHandleBuffer[0], (uint8_t *)&tmp,
        sizeof(tmp));
