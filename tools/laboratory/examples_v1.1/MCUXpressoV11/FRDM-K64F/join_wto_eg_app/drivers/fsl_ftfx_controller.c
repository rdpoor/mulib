...
enum _ftfx_ram_func_constants {
    kFTFx_RamFuncMaxSizeInWords =
        16U, /*!< The maximum size of execute-in-RAM function.*/
};
...
/*! @brief Internal function Flash asynchronous command sequence. Called by
 * driver APIs only*/
...
static status_t ftfx_check_resource_range(ftfx_config_t *config, uint32_t start,
                                          uint32_t lengthInBytes,
...
ftfx_check_flexram_function_option(ftfx_flexram_func_opt_t option);
...
static volatile uint32_t *const kFCCOBx =
    (volatile uint32_t *)(uint32_t)&FTFx_FCCOB3_REG;
...
status_t FTFx_API_UpdateFlexnvmPartitionStatus(ftfx_config_t *config) {
    ...
    flexnvmInfoIfrAddr = config->ifrDesc.resRange.dflashIfrStart +
    ...
    returnCode = FTFx_CMD_ReadResource(
        config, flexnvmInfoIfrAddr, (uint8_t *)&dataIFRReadOut,
...
#error "Cannot get FlexNVM memory partition info"
    ...
    config->eepromTotalSize =
        kEepromDensities[dataIFRReadOut.EEPROMDataSetSize];

    /* Fill out partitioned DFlash size */
    dataIFRReadOut.FlexNVMPartitionCode &= 0x0FU;
    config->flashDesc.totalSize =
        kDflashDensities[dataIFRReadOut.FlexNVMPartitionCode];
...
status_t FTFx_CMD_Erase(ftfx_config_t *config, uint32_t start,
    ...
    returnCode = ftfx_check_mem_range(config, start, lengthInBytes,
                                      config->opsConfig.addrAligment.sectorCmd);
    if (returnCode != kStatus_FTFx_Success) {
        return returnCode;
    }
...
status_t FTFx_CMD_EraseSectorNonBlocking(ftfx_config_t *config, uint32_t start,
    ...
    returnCode =
        ftfx_check_mem_range(config, start, lengthInBytes, aligmentInBytes);
    if (returnCode != kStatus_FTFx_Success) {
        return returnCode;
    }
    /* Validate the user key */
...
status_t FTFx_CMD_EraseAll(ftfx_config_t *config, uint32_t key) {
    ...
    if (returnCode == kStatus_FTFx_Success) {
        if (config->ifrDesc.resRange.dflashIfrStart !=
...
#if defined(FSL_FEATURE_FLASH_HAS_FLEX_NVM) && FSL_FEATURE_FLASH_HAS_FLEX_NVM
    /* Data flash IFR will be erased by erase all unsecure command, so we need
     * to update FlexNVM memory partition status synchronously */
    if (returnCode == kStatus_FTFx_Success) {
        if (config->ifrDesc.resRange.dflashIfrStart !=
            config->ifrDesc.resRange.pflashIfrStart) {
            returnCode = FTFx_API_UpdateFlexnvmPartitionStatus(config);
...
status_t FTFx_CMD_Program(ftfx_config_t *config, uint32_t start,
    ...
    /* Check the supplied address range. */
    returnCode =
        ftfx_check_mem_range(config, start, lengthInBytes, blockWriteUnitSize);
    if (returnCode != kStatus_FTFx_Success) {
        return returnCode;
    }
    ...
    if (config->ifrDesc.feature.has8ByteIdxSupport != 0U) {
        if (config->ifrDesc.feature.has4ByteIdxSupport != 0U) {
            if (((index == config->ifrDesc.idxInfo.mix8byteIdxStart) ||
                 ((index >=
                   ((uint32_t)config->ifrDesc.idxInfo.mix8byteIdxStart + 1U)) &&
...
status_t FTFx_CMD_ProgramSection(ftfx_config_t *config, uint32_t start,
    ...
    if ((FTFx->FCNFG & FTFx_FCNFG_RAMRDY_MASK) == 0U) {
        ...
        returnCode = FTFx_CMD_SetFlexramFunction(
            config, kFTFx_FlexramFuncOptAvailableAsRam);
        if (returnCode != kStatus_FTFx_Success) {
            return returnCode;
        }
    }
    ...
    while (lengthInBytes > 0U) {
        ...
        uint32_t endAddressOfCurrentSector =
            ALIGN_UP(programaddress, sectorSize);
        ...
        if ((lengthInBytes + programaddress) > endAddressOfCurrentSector) {
            lengthTobeProgrammedOfCurrentSector =
                endAddressOfCurrentSector - programaddress;
        } else {
        ...
        while (lengthTobeProgrammedOfCurrentSector > 0U) {
            ...
            if (lengthTobeProgrammedOfCurrentSector >
            ...
            (void)memcpy((uint8_t *)config->flexramBlockBase,
                         &srcaddress[currentOffset], programSizeOfCurrentPass);
            /* Set programaddress address of the data to be programmed */
            kFCCOBx[0] = BYTE2WORD_1_3(FTFx_PROGRAM_SECTION,
                                       programaddress + currentOffset);
            /* Set program size in terms of
             * FEATURE_FLASH_SECTION_CMD_ADDRESS_ALIGMENT */
...
status_t FTFx_CMD_ProgramPartition(ftfx_config_t *config,
    ...
    kFCCOBx[1] =
        BYTE2WORD_1_1_2(eepromDataSizeCode, flexnvmPartitionCode, 0xFFFFU);

    /* calling flash command sequence function to execute the command */
    returnCode = ftfx_command_sequence(config);

#if defined(FSL_FEATURE_FLASH_HAS_FLEX_NVM) && FSL_FEATURE_FLASH_HAS_FLEX_NVM
    /* Data flash IFR will be updated by program partition command during reset
     * sequence, so we just set reserved values for partitioned FlexNVM size
     * here */
...
status_t FTFx_CMD_ReadOnce(ftfx_config_t *config, uint32_t index, uint8_t *dst,
    ...
    if (returnCode == kStatus_FTFx_Success) {
        ...
        /* Note: Have to separate the first index from the rest if it equals 0
         *       to avoid a pointless comparison of unsigned int to 0 compiler
         * warning */
        if (config->ifrDesc.feature.has8ByteIdxSupport != 0U) {
            if (config->ifrDesc.feature.has4ByteIdxSupport != 0U) {
                if (((index == config->ifrDesc.idxInfo.mix8byteIdxStart) ||
                     ((index >=
                       ((uint32_t)config->ifrDesc.idxInfo.mix8byteIdxStart +
                        1U)) &&
                      (index <= config->ifrDesc.idxInfo.mix8byteIdxEnd))) &&
                    (lengthInBytes == 8U)) {
...
status_t FTFx_CMD_ReadResource(ftfx_config_t *config, uint32_t start,
    ...
    /* Check the supplied address range. */
    returnCode = ftfx_check_resource_range(config, readstart, readlengthBytes,
                                           aligmentInBytes, option);
    if (returnCode != kStatus_FTFx_Success) {
        return returnCode;
    }
...
status_t FTFx_CMD_VerifyErase(ftfx_config_t *config, uint32_t start,
    ...
    while (0U != remainingBytes) {
        ...
        /* Calculate the size to be verified, this flash does not support erase
         * and program across block. */
 ...
 */
status_t
FTFx_CMD_VerifyEraseAllExecuteOnlySegments(ftfx_config_t *config,
                                           ftfx_margin_value_t margin) {
    if (config == NULL) {
        return kStatus_FTFx_InvalidArgument;
    }

    /* preparing passing parameter to verify erase all execute-only segments
     * command */
    kFCCOBx[0] =
        BYTE2WORD_1_1_2(FTFx_VERIFY_ALL_EXECUTE_ONLY_SEGMENT, margin, 0xFFFFU);
...
status_t FTFx_CMD_VerifyProgram(ftfx_config_t *config, uint32_t start,
    ...
    while (0U != programlength) {
        ...
        kFCCOBx[2] =
            ftfx_read_word_from_byte_address((const uint8_t *)expectedData);
...
status_t FTFx_CMD_SecurityBypass(ftfx_config_t *config,
    ...
    if (0x02U != (registerValue & 0x03U)) {
        ...
        kFCCOBx[1] = BYTE2WORD_1_1_1_1(backdoorKey[0], backdoorKey[1],
                                       backdoorKey[2], backdoorKey[3]);
        kFCCOBx[2] = BYTE2WORD_1_1_1_1(backdoorKey[4], backdoorKey[5],
                                       backdoorKey[6], backdoorKey[7]);
...
status_t FTFx_REG_GetSecurityState(ftfx_config_t *config,
    ...
    if ((uint8_t)kFTFx_FsecRegCode_SEC_Unsecured ==
        (registerValue & FTFx_FSEC_SEC_MASK)) {
        ...
        *state = kFTFx_SecurityStateNotSecure;
        ...
        if ((uint8_t)kFTFx_FsecRegCode_KEYEN_Enabled ==
            (registerValue & FTFx_FSEC_KEYEN_MASK)) {
...
status_t FTFx_CMD_SwapControl(ftfx_config_t *config, uint32_t address,
    ...
    if ((address &
         ((uint32_t)FSL_FEATURE_FLASH_PFLASH_SWAP_CONTROL_CMD_ADDRESS_ALIGMENT -
          1u)) != 0U) {
        return kStatus_FTFx_AlignmentError;
    }

    /* Make sure address provided is in the lower half of Program flash but not
     * in the Flash Configuration Field */
    if ((address >= (config->flashDesc.totalSize / 2u)) ||
        ((address >= (uint32_t)kFTFx_PflashConfigAreaStart) &&
         (address <= (uint32_t)kFTFx_PflashConfigAreaEnd))) {
    ...
    returnInfo->currentSwapBlockStatus =
        (ftfx_swap_block_status_t)FTFx_FCCOB6_REG;
 ...
 */
static void ftfx_copy_run_command_to_ram(uint32_t *ftfxRunCommand) {
    assert(sizeof(s_ftfxRunCommandFunctionCode) <=
           ((uint32_t)kFTFx_RamFuncMaxSizeInWords * 4U));
    ...
    (void)memcpy(ftfxRunCommand, s_ftfxRunCommandFunctionCode,
...
static status_t ftfx_command_sequence(ftfx_config_t *config) {
    ...
    FTFx->FSTAT = FTFx_FSTAT_RDCOLERR_MASK | FTFx_FSTAT_ACCERR_MASK |
    ...
    callFtfxRunCommand_t callFtfxRunCommand =
        config->runCmdFuncAddr.callFlashCommand;

    /* We pass the ftfx_fstat address as a parameter to flash_run_comamnd()
     * instead of using pre-processed MICRO sentences or operating global
     * variable in flash_run_comamnd() to make sure that flash_run_command()
     * will be compiled into position-independent code (PIC). */
...
static status_t ftfx_check_mem_range(ftfx_config_t *config,
    ...
    if ((startAddress >= config->flashDesc.blockBase) &&
        ((startAddress + lengthInBytes) <=
         (config->flashDesc.blockBase + config->flashDesc.totalSize))) {
    ...
    FSL_FEATURE_FLASH_HAS_FLEX_NVM_ALIAS
             ...
             ((startAddress + lengthInBytes) <=
              (config->flashDesc.aliasBlockBase +
               config->flashDesc.totalSize))) {
        status = kStatus_FTFx_Success;
    } else {
...
static void ftfx_write_word_to_byte_address(uint8_t *dst, uint32_t word) {
    ...
    if (0U == ((uint32_t)writedst % 4U)) {
        *(uint32_t *)(uint32_t)writedst = word;
        ...
        /* Write 4 bytes into a non-4-byte aligned address memory, 1 byte one
         * time */
        for (uint32_t i = 0U; i < 4U; i++) {
...
static status_t ftfx_check_resource_range(ftfx_config_t *config, uint32_t start,
    ...
    if ((0U != (start & (alignmentBaseline - 1u))) ||
        (0U != (lengthInBytes & (alignmentBaseline - 1u)))) {
        return kStatus_FTFx_AlignmentError;
    }
    ...
    if (option == kFTFx_ResourceOptionVersionId) {
        ...
        if ((start >= config->ifrDesc.resRange.pflashIfrStart) &&
            (maxReadbleAddress < (config->ifrDesc.resRange.pflashIfrStart +
                                  config->ifrDesc.resRange.ifrMemSize))) {
        }
#if defined(FSL_FEATURE_FLASH_HAS_FLEX_NVM) && FSL_FEATURE_FLASH_HAS_FLEX_NVM
        /* read resource code from the  date flash IFR space */
        else if ((start >= config->ifrDesc.resRange.dflashIfrStart) &&
                 (maxReadbleAddress < (config->ifrDesc.resRange.dflashIfrStart +
                                       config->ifrDesc.resRange.ifrMemSize))) {
        }
#endif /* FSL_FEATURE_FLASH_HAS_FLEX_NVM */
#if defined(FSL_FEATURE_FLASH_HAS_SWAP_CONTROL_CMD) &&                         \
...
/*! @brief Validates the given swap control option.*/
static status_t ftfx_check_swap_control_option(ftfx_swap_control_opt_t option) {
    if ((option == kFTFx_SwapControlOptionIntializeSystem) ||
        (option == kFTFx_SwapControlOptionSetInUpdateState) ||
        (option == kFTFx_SwapControlOptionSetInCompleteState) ||
        (option == kFTFx_SwapControlOptionReportStatus) ||
