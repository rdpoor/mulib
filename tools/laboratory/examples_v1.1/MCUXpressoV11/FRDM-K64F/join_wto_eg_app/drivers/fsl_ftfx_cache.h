...
typedef union {
    uint32_t commadAddr;
    void (*callFlashCommand)(FTFx_REG32_ACCESS_TYPE base, uint32_t bitMask,
...
typedef struct _ftfx_cache_config {
    ...
    uint8_t reserved[3];
    function_bit_operation_ptr_t
        bitOperFuncAddr; /*!< An buffer point to the flash execute-in-RAM
                            function. */
} ftfx_cache_config_t;
 ...
 *
 * This function checks and initializes the Flash module for the other FTFx
 * cache APIs.
 *
 ...
 * @param isPreProcess The possible option used to control flash
 ...
 * @retval #kStatus_FTFx_Success API was executed successfully.
 * @retval #kStatus_FTFx_InvalidSpeculationOption An invalid speculation option
