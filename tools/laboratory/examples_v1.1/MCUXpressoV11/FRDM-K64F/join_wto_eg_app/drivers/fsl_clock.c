...
#define MCG_S_IRCST_VAL                                                        \
    (((uint32_t)MCG->S & (uint32_t)MCG_S_IRCST_MASK) >>                        \
     (uint32_t)MCG_S_IRCST_SHIFT)
#define MCG_S_CLKST_VAL                                                        \
    (((uint32_t)MCG->S & (uint32_t)MCG_S_CLKST_MASK) >>                        \
     (uint32_t)MCG_S_CLKST_SHIFT)
#define MCG_S_IREFST_VAL                                                       \
    (((uint32_t)MCG->S & (uint32_t)MCG_S_IREFST_MASK) >>                       \
     (uint32_t)MCG_S_IREFST_SHIFT)
#define MCG_S_PLLST_VAL                                                        \
    (((uint32_t)MCG->S & (uint32_t)MCG_S_PLLST_MASK) >>                        \
     (uint32_t)MCG_S_PLLST_SHIFT)
#define MCG_C1_FRDIV_VAL ((MCG->C1 & MCG_C1_FRDIV_MASK) >> MCG_C1_FRDIV_SHIFT)
#define MCG_C2_LP_VAL                                                          \
    (((uint32_t)MCG->C2 & (uint32_t)MCG_C2_LP_MASK) >>                         \
     (uint32_t)MCG_C2_LP_SHIFT)
#define MCG_C2_RANGE_VAL ((MCG->C2 & MCG_C2_RANGE_MASK) >> MCG_C2_RANGE_SHIFT)
#define MCG_SC_FCRDIV_VAL                                                      \
    ((MCG->SC & MCG_SC_FCRDIV_MASK) >> MCG_SC_FCRDIV_SHIFT)
#define MCG_S2_PLLCST_VAL                                                      \
    (((uint32_t)MCG->S2 & (uint32_t)MCG_S2_PLLCST_MASK) >>                     \
     (uint32_t)MCG_S2_PLLCST_SHIFT)
...
#define MCG_C4_DRST_DRS_VAL                                                    \
    ((MCG->C4 & MCG_C4_DRST_DRS_MASK) >> MCG_C4_DRST_DRS_SHIFT)
#define MCG_C7_PLL32KREFSEL_VAL                                                \
    ((MCG->C7 & MCG_C7_PLL32KREFSEL_MASK) >> MCG_C7_PLL32KREFSEL_SHIFT)
#define MCG_C5_PLLREFSEL0_VAL                                                  \
    ((MCG->C5 & MCG_C5_PLLREFSEL0_MASK) >> MCG_C5_PLLREFSEL0_SHIFT)
#define MCG_C11_PLLREFSEL1_VAL                                                 \
    ((MCG->C11 & MCG_C11_PLLREFSEL1_MASK) >> MCG_C11_PLLREFSEL1_SHIFT)
#define MCG_C11_PRDIV1_VAL                                                     \
    ((MCG->C11 & MCG_C11_PRDIV1_MASK) >> MCG_C11_PRDIV1_SHIFT)
#define MCG_C12_VDIV1_VAL                                                      \
    ((MCG->C12 & MCG_C12_VDIV1_MASK) >> MCG_C12_VDIV1_SHIFT)
#define MCG_C5_PRDIV0_VAL                                                      \
    ((uint8_t)(MCG->C5 & MCG_C5_PRDIV0_MASK) >> MCG_C5_PRDIV0_SHIFT)
#define MCG_C6_VDIV0_VAL                                                       \
    ((uint8_t)(MCG->C6 & MCG_C6_VDIV0_MASK) >> MCG_C6_VDIV0_SHIFT)

#define OSC_MODE_MASK                                                          \
    (MCG_C2_EREFS0_MASK | MCG_C2_HGO0_MASK | MCG_C2_RANGE0_MASK)

#define SIM_CLKDIV1_OUTDIV1_VAL                                                \
    ((SIM->CLKDIV1 & SIM_CLKDIV1_OUTDIV1_MASK) >> SIM_CLKDIV1_OUTDIV1_SHIFT)
#define SIM_CLKDIV1_OUTDIV2_VAL                                                \
    ((SIM->CLKDIV1 & SIM_CLKDIV1_OUTDIV2_MASK) >> SIM_CLKDIV1_OUTDIV2_SHIFT)
#define SIM_CLKDIV1_OUTDIV3_VAL                                                \
    ((SIM->CLKDIV1 & SIM_CLKDIV1_OUTDIV3_MASK) >> SIM_CLKDIV1_OUTDIV3_SHIFT)
#define SIM_CLKDIV1_OUTDIV4_VAL                                                \
    ((SIM->CLKDIV1 & SIM_CLKDIV1_OUTDIV4_MASK) >> SIM_CLKDIV1_OUTDIV4_SHIFT)
#define SIM_SOPT1_OSC32KSEL_VAL                                                \
    ((SIM->SOPT1 & SIM_SOPT1_OSC32KSEL_MASK) >> SIM_SOPT1_OSC32KSEL_SHIFT)
#define SIM_SOPT2_PLLFLLSEL_VAL                                                \
    ((SIM->SOPT2 & SIM_SOPT2_PLLFLLSEL_MASK) >> SIM_SOPT2_PLLFLLSEL_SHIFT)
 ...
 *
 * Delay function to wait FLL stable in FEI mode or FEE mode, should wait at
 * least 1ms. Every time changes FLL setting, should wait this time for FLL
 * stable.
 */
...
static void CLOCK_FllStableDelay(void) {
    /*
       ...
       100MHz at most, so this function could obtain the 1ms delay.
     */
...
/* Once user defines the MCG_USER_CONFIG_FLL_STABLE_DELAY_EN to use their own
 * delay function, he has to create his own CLOCK_FllStableDelay() function in
 * application code. Since the clock functions in this file would call the
 * CLOCK_FllStableDelay() regardless how it is defined.
 */
...
bool CLOCK_EnableUsbfs0Clock(clock_usb_src_t src, uint32_t freq) {
    ...
    if (kCLOCK_UsbSrcExt == src) {
        ...
        SIM->SOPT2 = ((SIM->SOPT2 &
                       ~(SIM_SOPT2_PLLFLLSEL_MASK | SIM_SOPT2_USBSRC_MASK)) |
                      (uint32_t)src);
    }
 ...
 *
 * This function gets the MCG output clock frequency in Hz based on the current
 * MCG register value.
 *
...
uint32_t CLOCK_GetFllFreq(void) {
    ...
    if ((((MCG->C2 & MCG_C2_LP_MASK) != 0U) ||
         ((MCG->S & MCG_S_PLLST_MASK) != 0U))) {
        ret = 0U;
    } else {
 ...
 *
 * This function gets the MCG PLL0 clock frequency in Hz based on the current
 * MCG register value.
 *
...
uint32_t CLOCK_GetPll0Freq(void) {
    ...
    if (((MCG->S & MCG_S_LOCK0_MASK)) == 0U) {
        ...
        /*
         * Please call CLOCK_SetXtal0Freq base on board setting before using
         * OSC0 clock. Please call CLOCK_SetXtal1Freq base on board setting
         * before using OSC1 clock.
         */
        assert(mcgpll0clk);

        mcgpll0prdiv =
            ((uint8_t)FSL_FEATURE_MCG_PLL_PRDIV_BASE + MCG_C5_PRDIV0_VAL);
        mcgpll0clk /= (uint32_t)mcgpll0prdiv;
        mcgpll0vdiv =
            ((uint8_t)FSL_FEATURE_MCG_PLL_VDIV_BASE + MCG_C6_VDIV0_VAL);
 ...
 * clock should not be changed in FEE/FBE/BLPE/PBE/PEE modes, do not call this
 * function in these modes.
 *
 * param oscsel MCG external reference clock source, MCG_C7[OSCSEL].
 * retval kStatus_MCG_SourceUsed Because the external reference clock is used as
 * a clock source, the configuration should not be changed. Otherwise, a glitch
 * occurs. retval kStatus_Success External reference clock set successfully.
 */
status_t CLOCK_SetExternalRefClkConfig(mcg_oscsel_t oscsel) {
    ...
    /* If change MCG_C7[OSCSEL] and external reference clock is system clock
     * source, return error. */
    ...
    if (needDelay) {
        /* ERR009878 Delay at least 50 micro-seconds for external clock change
         * valid. */
        i = 1500U;
        while (0U != (i--)) {
            __NOP();
 ...
 *
 * This function sets the \c MCGIRCLK base on parameters. It also selects the
 * IRC source. If the fast IRC is used, this function sets the fast IRC divider.
 * This function also sets whether the \c MCGIRCLK is enabled in stop mode.
 * Calling this function in FBI/PBI/BLPI modes may change the system clock. As a
 * result, using the function in these modes it is not allowed.
 *
 * param enableMode MCGIRCLK enable mode, OR'ed value of ref
 * _mcg_irclk_enable_mode. param ircs       MCGIRCLK clock source, choose fast
 * or slow. param fcrdiv     Fast IRC divider setting (\c FCRDIV). retval
 * kStatus_MCG_SourceUsed Because the internal reference clock is used as a
 * clock source, the configuration should not be changed. Otherwise, a glitch
 * occurs. retval kStatus_Success MCGIRCLK configuration finished successfully.
 */
status_t CLOCK_SetInternalRefClkConfig(uint8_t enableMode, mcg_irc_mode_t ircs,
                                       uint8_t fcrdiv) {
    ...
    if ((uint32_t)kMCG_ClkOutStatInt == mcgOutClkState) {
        ...
        if (((kMCG_IrcFast == curIrcs) && (fcrdiv != curFcrdiv)) ||
            (ircs != curIrcs)) {
            return kStatus_MCG_SourceUsed;
        }
    }
    ...
    if (fcrdiv != curFcrdiv) {
        ...
        if (((0U != (MCG->C1 & MCG_C1_IRCLKEN_MASK)) ||
             (mcgOutClkState == (uint32_t)kMCG_ClkOutStatInt)) &&
        ...
        /* Update FCRDIV. */
        MCG->SC = (uint8_t)(MCG->SC & ~(MCG_SC_FCRDIV_MASK | MCG_SC_ATMF_MASK |
                                        MCG_SC_LOCS0_MASK)) |
                  MCG_SC_FCRDIV(fcrdiv);
    }

    /* Set internal reference clock selection. */
    MCG->C2 = (uint8_t)((MCG->C2 & ~MCG_C2_IRCS_MASK) | (MCG_C2_IRCS(ircs)));
    MCG->C1 =
        (uint8_t)((MCG->C1 & ~(MCG_C1_IRCLKEN_MASK | MCG_C1_IREFSTEN_MASK)) |
                  (uint8_t)enableMode);

    /* If MCGIRCLK is used, need to wait for MCG_S_IRCST. */
    if ((mcgOutClkState == (uint32_t)kMCG_ClkOutStatInt) ||
        (0U != (enableMode & (uint32_t)kMCG_IrclkEnable))) {
 ...
 * VCO divider (\c VDIV) to generate a desired PLL output frequency. It returns
 * the closest frequency match with the corresponding \c PRDIV/VDIV returned
 * from parameters. If a desired frequency is not valid, this function returns
 * 0.
 *
 ...
 */
uint32_t CLOCK_CalcPllDiv(uint32_t refFreq, uint32_t desireFreq, uint8_t *prdiv,
                          uint8_t *vdiv) {
    uint8_t ret_prdiv; /* PRDIV to return. */
    uint8_t ret_vdiv;  /* VDIV to return.  */
    uint8_t prdiv_min; /* Min PRDIV value to make reference clock in allowed
                          range. */
    uint8_t prdiv_max; /* Max PRDIV value to make reference clock in allowed
                          range. */
    ...
    uint32_t diff =
        0xFFFFFFFFU;  /* Difference between desireFreq and return frequency. */
 ...
 * clock is enabled before calling this function and that the PLL0 is not used
 * as a clock source. The function CLOCK_CalcPllDiv gets the correct PLL divider
 * values.
 *
...
void CLOCK_EnablePll0(mcg_pll_config_t const *config) {
    ...
    MCG->C6 =
        (uint8_t)((MCG->C6 & ~MCG_C6_VDIV0_MASK) | MCG_C6_VDIV0(config->vdiv));

    /* Set enable mode. */
    MCG->C5 |=
        ((uint8_t)kMCG_PllEnableIndependent | (uint8_t)config->enableMode);
 ...
 *
 * This function sets the OSC0 clock monitor mode. See ref mcg_monitor_mode_t
 * for details.
 *
 ...
 *
 * This function sets the RTC OSC clock monitor mode. See ref mcg_monitor_mode_t
 * for details.
 *
 * param mode Monitor mode to set.
 */
 ...
 *
 * This function sets the PLL0 clock monitor mode. See ref mcg_monitor_mode_t
 * for details.
 *
 * param mode Monitor mode to set.
 */
 ...
 *
 * This function clears the MCG clock lock lost status. The parameter is a
 * logical OR value of the flags to clear. See ref _mcg_status_flags_t.
 *
 * Example:
 * code
 ...
 */
void CLOCK_SetSlowIrcFreq(uint32_t freq) { s_slowIrcFreq = freq; }
 ...
 */
void CLOCK_SetFastIrcFreq(uint32_t freq) { s_fastIrcFreq = freq; }

/*!
 * brief Auto trims the internal reference clock.
 *
 * This function trims the internal reference clock by using the external clock.
 * If successful, it returns the kStatus_Success and the frequency after
 ...
 * retval kStatus_MCG_AtmBusClockInvalid The bus clock is not in allowed range
 * for the ATM. retval kStatus_MCG_AtmDesiredFreqInvalid MCGIRCLK could not be
 * trimmed to the desired frequency. retval kStatus_MCG_AtmIrcUsed Could not
 * trim because MCGIRCLK is used as a bus clock source. retval
...
status_t CLOCK_TrimInternalRefClk(uint32_t extFreq, uint32_t desireFreq,
    ...
    else if ((desireFreq < trimRange[atms][0]) ||
             (desireFreq > trimRange[atms][1])) {
     ...
     */
    else if (MCG_S_IREFST(kMCG_FllSrcInternal) ==
 ...
 * Passing NULL does not cause a delay. retval kStatus_MCG_ModeUnreachable Could
 * not switch to the target mode. retval kStatus_Success Switched to the target
 * mode successfully. note If p dmx32 is set to kMCG_Dmx32Fine, the slow IRC
 * must not be trimmed to a frequency above 32768 Hz.
 */
status_t CLOCK_SetFeiMode(mcg_dmx32_t dmx32, mcg_drs_t drs,
    ...
    if (!((kMCG_ModeFEI == mode) || (kMCG_ModeFBI == mode) ||
          (kMCG_ModeFBE == mode) || (kMCG_ModeFEE == mode))) {
...
status_t CLOCK_SetFeeMode(uint8_t frdiv, mcg_dmx32_t dmx32, mcg_drs_t drs,
...
#if (defined(MCG_CONFIG_CHECK_PARAM) && MCG_CONFIG_CHECK_PARAM)
    mcg_mode_t mode = CLOCK_GetMode();
    if (!((kMCG_ModeFEE == mode) || (kMCG_ModeFBI == mode) ||
          (kMCG_ModeFBE == mode) || (kMCG_ModeFEI == mode))) {
        return kStatus_MCG_ModeUnreachable;
    }
#endif
    mcg_c4 = MCG->C4;
    ...
    /* Set CLKS and IREFS. */
    MCG->C1 = (uint8_t)((MCG->C1 & ~(MCG_C1_CLKS_MASK | MCG_C1_FRDIV_MASK |
                                     MCG_C1_IREFS_MASK)) |
                        (MCG_C1_CLKS(kMCG_ClkOutSrcOut)         /* CLKS = 0 */
 ...
 */
status_t CLOCK_SetFbiMode(mcg_dmx32_t dmx32, mcg_drs_t drs,
                          void (*fllStableDelay)(void)) {
    uint8_t mcg_c4;
    bool change_drs = false;
 ...
 * platforms,  it is possible to choose the external PLL directly, which renders
 * the configuration structure not necessary. In this case, pass in NULL. For
 * example: CLOCK_SetPbeMode(kMCG_OscselOsc, kMCG_PllClkSelExtPll, NULL);
 */
status_t CLOCK_SetPbeMode(mcg_pll_clk_select_t pllcs,
    ...
    /*
       ...
       but with this workflow, the source mode could be all modes except
       PEI/PBI.
     */
    MCG->C2 &= (uint8_t)(~MCG_C2_LP_MASK); /* Disable lowpower. */
    ...
    while (
        (MCG->S & (MCG_S_IREFST_MASK | MCG_S_CLKST_MASK)) !=
 ...
 *
 * This function switches the MCG from external modes (PEE/PBE/BLPE/FEE) to the
 * FBE mode quickly. The external clock is used as the system clock source and
 * PLL is disabled. However, the FLL settings are not configured. This is a lite
 * function with a small code size, which is useful during the mode switch. For
 ...
 * retval kStatus_MCG_ModeInvalid If the current mode is not an external mode,
 ...
 *
 * This function switches the MCG from internal modes (PEI/PBI/BLPI/FEI) to the
 * FBI mode quickly. The MCGIRCLK is used as the system clock source and PLL is
 * disabled. However, FLL settings are not configured. This is a lite function
 * with a small code size, which is useful during the mode switch. For example,
 * to switch from PEI mode to FEE mode:
 *
 * code
 ...
 *
 * retval kStatus_Success Switched successfully.
 * retval kStatus_MCG_ModeInvalid If the current mode is not an internal mode,
 * do not call this function.
 */
 ...
 *
 * This function sets the MCG to FEI mode from the reset mode. It can also be
 * used to set up MCG during system boot up.
 *
 * param  dmx32  DMX32 in FEI mode.
 * param  drs The DCO range selection.
 * param  fllStableDelay Delay function to ensure that the FLL is stable.
 ...
 * to frequency above 32768 Hz.
 */
status_t CLOCK_BootToFeiMode(mcg_dmx32_t dmx32, mcg_drs_t drs,
                             void (*fllStableDelay)(void)) {
 ...
 *
 * This function sets MCG to FEE mode from the reset mode. It can also be used
 * to set up the MCG during system boot up.
 *
 ...
 * retval kStatus_Success Switched to the target mode successfully.
 */
status_t CLOCK_BootToFeeMode(mcg_oscsel_t oscsel, uint8_t frdiv,
                             mcg_dmx32_t dmx32, mcg_drs_t drs,
                             void (*fllStableDelay)(void)) {
 ...
 *
 * This function sets the MCG to BLPI mode from the reset mode. It can also be
 * used to set up the MCG during system boot up.
 *
 * param  fcrdiv Fast IRC divider, FCRDIV.
 * param  ircs   The internal reference clock to select, IRCS.
 * param  ircEnableMode  The MCGIRCLK enable mode, OR'ed value of ref
 ...
 *
 * This function sets the MCG to BLPE mode from the reset mode. It can also be
 * used to set up the MCG during system boot up.
 *
 ...
 *
 * This function sets the MCG to PEE mode from reset mode. It can also be used
 * to set up the MCG during system boot up.
 *
 * param   oscsel OSC clock select, MCG_C7[OSCSEL].
 * param   pllcs  The PLL selection, PLLCS.
 ...
 * retval kStatus_Success Switched to the target mode successfully.
 */
status_t CLOCK_BootToPeeMode(mcg_oscsel_t oscsel, mcg_pll_clk_select_t pllcs,
                             mcg_pll_config_t const *config) {
    assert(config);
...
/*
   ...
   1. Current mode FEI, next mode is mcgModeMatrix[FEI][PEE] = FBE, so swith to
   FBE.
   2. Current mode FBE, next mode is mcgModeMatrix[FBE][PEE] = PBE, so swith to
   PBE.
   3. Current mode PBE, next mode is mcgModeMatrix[PBE][PEE] = PEE, so swith to
   PEE. Thus the MCG mode has changed from FEI to PEE.
 */
static const mcg_mode_t mcgModeMatrix[8][8] = {
    {kMCG_ModeFEI, kMCG_ModeFBI, kMCG_ModeFBI, kMCG_ModeFEE, kMCG_ModeFBE,
     kMCG_ModeFBE, kMCG_ModeFBE, kMCG_ModeFBE}, /* FEI */
    {kMCG_ModeFEI, kMCG_ModeFBI, kMCG_ModeBLPI, kMCG_ModeFEE, kMCG_ModeFBE,
     kMCG_ModeFBE, kMCG_ModeFBE, kMCG_ModeFBE}, /* FBI */
    {kMCG_ModeFBI, kMCG_ModeFBI, kMCG_ModeBLPI, kMCG_ModeFBI, kMCG_ModeFBI,
     kMCG_ModeFBI, kMCG_ModeFBI, kMCG_ModeFBI}, /* BLPI */
    {kMCG_ModeFEI, kMCG_ModeFBI, kMCG_ModeFBI, kMCG_ModeFEE, kMCG_ModeFBE,
     kMCG_ModeFBE, kMCG_ModeFBE, kMCG_ModeFBE}, /* FEE */
    {kMCG_ModeFEI, kMCG_ModeFBI, kMCG_ModeFBI, kMCG_ModeFEE, kMCG_ModeFBE,
     kMCG_ModeBLPE, kMCG_ModePBE, kMCG_ModePBE}, /* FBE */
    {kMCG_ModeFBE, kMCG_ModeFBE, kMCG_ModeFBE, kMCG_ModeFBE, kMCG_ModeFBE,
     kMCG_ModeBLPE, kMCG_ModePBE, kMCG_ModePBE}, /* BLPE */
    {kMCG_ModeFBE, kMCG_ModeFBE, kMCG_ModeFBE, kMCG_ModeFBE, kMCG_ModeFBE,
     kMCG_ModeBLPE, kMCG_ModePBE, kMCG_ModePEE}, /* PBE */
    {kMCG_ModePBE, kMCG_ModePBE, kMCG_ModePBE, kMCG_ModePBE, kMCG_ModePBE,
     kMCG_ModePBE, kMCG_ModePBE, kMCG_ModePBE} /* PEE */
 ...
 * return Return kStatus_Success if switched successfully; Otherwise, it returns
 * an error code #_mcg_status.
 *
...
status_t CLOCK_SetMcgConfig(const mcg_config_t *config) {
    ...
    /* Re-configure MCGIRCLK, if MCGIRCLK is used as system clock source, then
     * change to FEI/PEI first. */
    if (MCG_S_CLKST_VAL == (uint8_t)kMCG_ClkOutStatInt) {
        MCG->C2 &= ~(uint8_t)MCG_C2_LP_MASK; /* Disable lowpower. */

        {
            (void)CLOCK_SetFeiMode(config->dmx32, config->drs,
                                   CLOCK_FllStableDelay);
        }
    }

    /* Configure MCGIRCLK. */
    (void)CLOCK_SetInternalRefClkConfig(config->irclkEnableMode, config->ircs,
                                        config->fcrdiv);
    ...
    do {
        ...
        case kMCG_ModeFEI:
            ...
            break;
        case kMCG_ModeFEE:
            status = CLOCK_SetFeeMode(config->frdiv, config->dmx32, config->drs,
                                      CLOCK_FllStableDelay);
            break;
        case kMCG_ModeFBI:
            ...
            break;
        case kMCG_ModeFBE:
            status = CLOCK_SetFbeMode(config->frdiv, config->dmx32, config->drs,
                                      NULL);
            break;
        case kMCG_ModeBLPI:
        ...
        case kMCG_ModePBE:
             ...
             * EXT here. */
            if ((kMCG_ModePEE == config->mcgMode) ||
                (kMCG_ModePBE == config->mcgMode)) {
                {
