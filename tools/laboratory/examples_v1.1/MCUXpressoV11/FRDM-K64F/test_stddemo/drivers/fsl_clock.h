 ...
 *  1. MCGIRCLK settings, such as the source, divider, and the trim value should
 * not change when MCGIRCLK is used as a system clock source.
 *  2. MCG_C7[OSCSEL] should not be changed  when the external reference clock
 * is used as a system clock source. For example, in FBE/BLPE/PBE modes.
 *  3. The users should only switch between the supported clock modes.
 *
 * MCG functions check the parameter and MCG status before setting, if not
 * allowed to change, the functions return error. The parameter checking
 * increases code size, if code size is a critical requirement, change
 * #MCG_CONFIG_CHECK_PARAM to 0 to disable parameter checking.
 */
 ...
 *
 * The XTAL0/EXTAL0 (OSC0) clock frequency in Hz. When the clock is set up, use
 * the function CLOCK_SetXtal0Freq to set the value in the clock driver. For
 * example, if XTAL0 is 8 MHz:
 * @code
 * Set up the OSC0
 * CLOCK_InitOsc0(...);
 ...
 * set up the OSC0 using the CLOCK_InitOsc0. All other cores need to call the
 * CLOCK_SetXtal0Freq to get a valid clock frequency.
 */
extern volatile uint32_t g_xtal0Freq;

/*! @brief External XTAL32/EXTAL32/RTC_CLKIN clock frequency.
 *
 * The XTAL32/EXTAL32/RTC_CLKIN clock frequency in Hz. When the clock is set up,
 * use the function CLOCK_SetXtal32Freq to set the value in the clock driver.
 *
 * This is important for the multicore platforms where only one core needs to
...
typedef enum _clock_name {
    ...
    kCLOCK_Osc0ErClkUndiv, /*!< OSC0 external reference undivided
                              clock(OSC0ERCLK_UNDIV). */
...
typedef enum _clock_usb_src {
    kCLOCK_UsbSrcPll0 =
        SIM_SOPT2_USBSRC(1U) | SIM_SOPT2_PLLFLLSEL(1U), /*!< Use PLL0.      */
    kCLOCK_UsbSrcIrc48M =
        SIM_SOPT2_USBSRC(1U) | SIM_SOPT2_PLLFLLSEL(3U), /*!< Use IRC48M.    */
...
#define CLK_GATE_ABSTRACT_REG_OFFSET(x)                                        \
    (((x)&CLK_GATE_REG_OFFSET_MASK) >> CLK_GATE_REG_OFFSET_SHIFT)
#define CLK_GATE_ABSTRACT_BITS_SHIFT(x)                                        \
    (((x)&CLK_GATE_BIT_SHIFT_MASK) >> CLK_GATE_BIT_SHIFT_SHIFT)
...
typedef struct _oscer_config {
    uint8_t enableMode; /*!< OSCERCLK enable mode. OR'ed value of @ref
                           _oscer_enable_mode. */
...
typedef enum _mcg_dmx32 {
    ...
    kMCG_Dmx32Fine /*!< DCO is fine-tuned for maximum frequency with 32.768 kHz
                      reference */
} mcg_dmx32_t;
...
enum {
    kStatus_MCG_ModeUnreachable =
        MAKE_STATUS(kStatusGroup_MCG, 0U), /*!< Can't switch to target mode. */
    kStatus_MCG_ModeInvalid =
        MAKE_STATUS(kStatusGroup_MCG, 1U), /*!< Current mode invalid for the
                                              specific function. */
    kStatus_MCG_AtmBusClockInvalid =
        MAKE_STATUS(kStatusGroup_MCG, 2U), /*!< Invalid bus clock for ATM. */
    kStatus_MCG_AtmDesiredFreqInvalid = MAKE_STATUS(
        kStatusGroup_MCG, 3U), /*!< Invalid desired frequency for ATM. */
    kStatus_MCG_AtmIrcUsed =
        MAKE_STATUS(kStatusGroup_MCG, 4U), /*!< IRC is used when using ATM. */
    kStatus_MCG_AtmHardwareFail = MAKE_STATUS(
        kStatusGroup_MCG, 5U), /*!< Hardware fail occurs during ATM. */
    kStatus_MCG_SourceUsed =
        MAKE_STATUS(kStatusGroup_MCG, 6U) /*!< Can't change the clock source
...
enum {
    ...
    kMCG_IrclkEnableInStop =
        MCG_C1_IREFSTEN_MASK /*!< MCGIRCLK enable in stop mode. */
};

/*! @brief MCG PLL clock enable mode definition. Enumeration
 * _mcg_pll_enable_mode */
enum {
    kMCG_PllEnableIndependent =
        MCG_C5_PLLCLKEN0_MASK, /*!< MCGPLLCLK enable independent of the
    ...
    kMCG_PllEnableInStop =
        MCG_C5_PLLSTEN0_MASK /*!< MCGPLLCLK enable in STOP mode. */
};
...
typedef struct _mcg_pll_config {
    uint8_t enableMode; /*!< Enable mode. OR'ed value of enumeration
                           _mcg_pll_enable_mode. */
...
static inline void CLOCK_EnableClock(clock_ip_name_t name) {
    ...
    (*(volatile uint32_t *)regAddr) |=
        (1UL << CLK_GATE_ABSTRACT_BITS_SHIFT((uint32_t)name));
}
...
static inline void CLOCK_SetEr32kClock(uint32_t src) {
    SIM->SOPT1 =
        ((SIM->SOPT1 & ~SIM_SOPT1_OSC32KSEL_MASK) | SIM_SOPT1_OSC32KSEL(src));
}
...
static inline void CLOCK_SetSdhc0Clock(uint32_t src) {
    SIM->SOPT2 =
        ((SIM->SOPT2 & ~SIM_SOPT2_SDHCSRC_MASK) | SIM_SOPT2_SDHCSRC(src));
}
...
static inline void CLOCK_SetEnetTime0Clock(uint32_t src) {
    SIM->SOPT2 =
        ((SIM->SOPT2 & ~SIM_SOPT2_TIMESRC_MASK) | SIM_SOPT2_TIMESRC(src));
}
...
static inline void CLOCK_SetRmii0Clock(uint32_t src) {
    SIM->SOPT2 =
        ((SIM->SOPT2 & ~SIM_SOPT2_RMIISRC_MASK) | SIM_SOPT2_RMIISRC(src));
}
...
static inline void CLOCK_SetTraceClock(uint32_t src) {
    SIM->SOPT2 = ((SIM->SOPT2 & ~SIM_SOPT2_TRACECLKSEL_MASK) |
                  SIM_SOPT2_TRACECLKSEL(src));
}
...
static inline void CLOCK_SetPllFllSelClock(uint32_t src) {
    SIM->SOPT2 =
        ((SIM->SOPT2 & ~SIM_SOPT2_PLLFLLSEL_MASK) | SIM_SOPT2_PLLFLLSEL(src));
}
...
static inline void CLOCK_SetClkOutClock(uint32_t src) {
    SIM->SOPT2 =
        ((SIM->SOPT2 & ~SIM_SOPT2_CLKOUTSEL_MASK) | SIM_SOPT2_CLKOUTSEL(src));
}
...
static inline void CLOCK_SetRtcClkOutClock(uint32_t src) {
    SIM->SOPT2 = ((SIM->SOPT2 & ~SIM_SOPT2_RTCCLKOUTSEL_MASK) |
                  SIM_SOPT2_RTCCLKOUTSEL(src));
}
 ...
 *
 * Set the SIM_CLKDIV1[OUTDIV1], SIM_CLKDIV1[OUTDIV2], SIM_CLKDIV1[OUTDIV3],
 * SIM_CLKDIV1[OUTDIV4].
 *
 ...
 */
static inline void CLOCK_SetOutDiv(uint32_t outdiv1, uint32_t outdiv2,
 ...
 *
 * The system level clocks (core clock, bus clock, flexbus clock and flash
 * clock) must be in allowed ranges. During MCG clock mode switch, the MCG
 * output clock changes then the system level clocks may be out of range. This
 * function could be used before MCG mode change, to make sure system level
 * clocks are in allowed range.
 *
 */
static inline void CLOCK_SetSimSafeDivs(void) { SIM->CLKDIV1 = 0x01240000U; }
 ...
 *
 * Enabling the MCG low power disables the PLL and FLL in bypass modes. In other
 * words, in FBE and PBE modes, enabling low power sets the MCG to BLPE mode. In
 * FBI and PBI modes, enabling low power sets the MCG to BLPI mode. When
 * disabling the MCG low power, the PLL or FLL are enabled based on MCG
 * settings.
 *
 ...
 *
 * @param enableMode MCGIRCLK enable mode, OR'ed value of the enumeration
 * _mcg_irclk_enable_mode.
 * @param ircs       MCGIRCLK clock source, choose fast or slow.
 * @param fcrdiv     Fast IRC divider setting (\c FCRDIV).
 * @retval kStatus_MCG_SourceUsed Because the internal reference clock is used
 * as a clock source, the configuration should not be changed. Otherwise, a
 * glitch occurs.
 ...
 * @retval kStatus_MCG_SourceUsed Because the external reference clock is used
 * as a clock source, the configuration should not be changed. Otherwise, a
 * glitch occurs.
 ...
 *
 * Sets the FLL external reference clock divider value, the register
 * MCG_C1[FRDIV].
 *
 ...
 *
 * This function sets the RTC OSC clock monitor mode. See @ref
 * mcg_monitor_mode_t for details.
 *
 * @param mode Monitor mode to set.
 */
 ...
 * logical OR value of the flags to clear. See the enumeration
 * _mcg_status_flags_t.
 *
 * Example:
 * @code
 ...
 * This is an example to enable the OSCERCLK in normal and stop modes and also
 set
 ...
 * @param config Pointer to the configuration structure.
 */
static inline void OSC_SetExtRefClkConfig(OSC_Type *base,
 ...
 *
 * @param base   OSC peripheral address.
 * @param capLoad OR'ed value for the capacitor load option, see \ref
 _osc_cap_load.
 *
 * Example:
   @code
...
static inline void OSC_SetCapLoad(OSC_Type *base, uint8_t capLoad) {
    ...
    reg &= (uint8_t)(~(OSC_CR_SC2P_MASK | OSC_CR_SC4P_MASK | OSC_CR_SC8P_MASK |
                       OSC_CR_SC16P_MASK));
    reg |= capLoad;
 ...
 */
static inline void CLOCK_SetXtal0Freq(uint32_t freq) { g_xtal0Freq = freq; }
 ...
 */
static inline void CLOCK_SetXtal32Freq(uint32_t freq) { g_xtal32Freq = freq; }
/* @} */
 ...
 * for the ATM.
 * @retval kStatus_MCG_AtmDesiredFreqInvalid MCGIRCLK could not be trimmed to
 * the desired frequency.
 * @retval kStatus_MCG_AtmIrcUsed Could not trim because MCGIRCLK is used as a
 * bus clock source.
 ...
 * Passing NULL does not cause a delay.
 ...
 * @param  ircEnableMode  The MCGIRCLK enable mode, OR'ed value of the
 * enumeration _mcg_irclk_enable_mode.
 *
 ...
 * @return Return kStatus_Success if switched successfully; Otherwise, it
 * returns an error code _mcg_status.
 *
