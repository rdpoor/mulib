...
typedef enum _smc_power_mode_protection {
#if (defined(FSL_FEATURE_SMC_HAS_VERY_LOW_LEAKAGE_STOP_MODE) &&                \
     FSL_FEATURE_SMC_HAS_VERY_LOW_LEAKAGE_STOP_MODE)
    kSMC_AllowPowerModeVlls =
        SMC_PMPROT_AVLLS_MASK, /*!< Allow Very-low-leakage Stop Mode. */
#endif
#if (defined(FSL_FEATURE_SMC_HAS_LOW_LEAKAGE_STOP_MODE) &&                     \
    ...
    kSMC_AllowPowerModeVlp =
        SMC_PMPROT_AVLP_MASK, /*!< Allow Very-Low-power Mode.        */
#if (defined(FSL_FEATURE_SMC_HAS_HIGH_SPEED_RUN_MODE) &&                       \
     FSL_FEATURE_SMC_HAS_HIGH_SPEED_RUN_MODE)
    kSMC_AllowPowerModeHsrun =
        SMC_PMPROT_AHSRUN_MASK, /*!< Allow High-speed Run mode.        */
...
typedef enum _smc_power_state {
    kSMC_PowerStateRun = 0x01U
                         << 0U, /*!< 0000_0001 - Current power mode is RUN   */
    kSMC_PowerStateStop = 0x01U
                          << 1U, /*!< 0000_0010 - Current power mode is STOP  */
    kSMC_PowerStateVlpr = 0x01U
                          << 2U, /*!< 0000_0100 - Current power mode is VLPR  */
    kSMC_PowerStateVlpw = 0x01U
                          << 3U, /*!< 0000_1000 - Current power mode is VLPW  */
    kSMC_PowerStateVlps = 0x01U
                          << 4U, /*!< 0001_0000 - Current power mode is VLPS  */
#if (defined(FSL_FEATURE_SMC_HAS_LOW_LEAKAGE_STOP_MODE) &&                     \
     FSL_FEATURE_SMC_HAS_LOW_LEAKAGE_STOP_MODE)
    kSMC_PowerStateLls = 0x01U
                         << 5U, /*!< 0010_0000 - Current power mode is LLS   */
#endif                          /* FSL_FEATURE_SMC_HAS_LOW_LEAKAGE_STOP_MODE */
#if (defined(FSL_FEATURE_SMC_HAS_VERY_LOW_LEAKAGE_STOP_MODE) &&                \
     FSL_FEATURE_SMC_HAS_VERY_LOW_LEAKAGE_STOP_MODE)
    kSMC_PowerStateVlls = 0x01U
                          << 6U, /*!< 0100_0000 - Current power mode is VLLS  */
#endif
#if (defined(FSL_FEATURE_SMC_HAS_HIGH_SPEED_RUN_MODE) &&                       \
     FSL_FEATURE_SMC_HAS_HIGH_SPEED_RUN_MODE)
    kSMC_PowerStateHsrun = 0x01U
...
#if (defined(FSL_FEATURE_SMC_USE_VLLSCTRL_REG) &&                              \
     FSL_FEATURE_SMC_USE_VLLSCTRL_REG) ||                                      \
    (defined(FSL_FEATURE_SMC_USE_STOPCTRL_VLLSM) &&                            \
     FSL_FEATURE_SMC_USE_STOPCTRL_VLLSM) ||                                    \
    (defined(FSL_FEATURE_SMC_HAS_LLS_SUBMODE) &&                               \
     FSL_FEATURE_SMC_HAS_LLS_SUBMODE)
/*!
...
typedef enum _smc_partial_stop_mode {
    ...
    kSMC_PartialStop1 =
        1U, /*!< Partial Stop with both system and bus clocks disabled*/
    kSMC_PartialStop2 =
        2U, /*!< Partial Stop with system clock disabled and bus clock enabled*/
...
enum {
    kStatus_SMC_StopAbort =
        MAKE_STATUS(kStatusGroup_POWER, 0) /*!< Entering Stop mode is abort*/
};
...
#if (defined(FSL_FEATURE_SMC_HAS_LLS_SUBMODE) &&                               \
     FSL_FEATURE_SMC_HAS_LLS_SUBMODE) ||                                       \
...
typedef struct _smc_power_mode_vlls_config {
    ...
    bool enablePorDetectInVlls0; /*!< Enable Power on reset detect in VLLS mode
                                  */
#endif
#if (defined(FSL_FEATURE_SMC_HAS_RAM2_POWER_OPTION) &&                         \
     FSL_FEATURE_SMC_HAS_RAM2_POWER_OPTION)
 ...
 */
static inline void SMC_GetVersionId(SMC_Type *base,
 ...
 * use SMC_SetPowerModeProtection(kSMC_AllowPowerModeVlls |
 * kSMC_AllowPowerModeVlps). To allow all modes, use
 ...
 * switches the power mode, it should always check the status to check whether
 * it runs into the specified mode or not. The application  should  check this
 * mode before switching to a different mode. The system  requires that only
 * certain modes can switch to other specific modes. See the reference manual
 * for details and the smc_power_state_t for information about the power status.
 *
 * @param base SMC peripheral base address.
