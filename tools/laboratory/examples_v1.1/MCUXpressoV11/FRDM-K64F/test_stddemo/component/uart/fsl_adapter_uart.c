...
static hal_uart_state_t
    *s_UartState[sizeof(s_UartAdapterBase) / sizeof(UART_Type *)];
#endif
...
#if (defined(HAL_UART_TRANSFER_MODE) && (HAL_UART_TRANSFER_MODE > 0U))
static void HAL_UartCallback(UART_Type *base, uart_handle_t *handle,
...
static void HAL_UartInterruptHandle(uint8_t instance) {
    ...
    if ((0U != ((uint32_t)kUART_RxDataRegFullFlag & status)) &&
        (0U != (UART_GetEnabledInterrupts(s_UartAdapterBase[instance]) &
                (uint32_t)kUART_RxDataRegFullInterruptEnable))) {
        clearStatus |=
            (uint32_t)kUART_RxDataRegFullFlag | (uint32_t)kUART_RxOverrunFlag;
        if (NULL != uartHandle->rx.buffer) {
            uartHandle->rx.buffer[uartHandle->rx.bufferSofar++] =
                UART_ReadByte(s_UartAdapterBase[instance]);
            if (uartHandle->rx.bufferSofar >= uartHandle->rx.bufferLength) {
                UART_DisableInterrupts(
                    s_UartAdapterBase[instance],
                    (uint32_t)kUART_RxDataRegFullInterruptEnable |
    ...
    if ((0U != ((uint32_t)kUART_TxDataRegEmptyFlag & status)) &&
        ...
        clearStatus |= (uint32_t)kUART_TxDataRegEmptyFlag |
                       (uint32_t)kUART_TransmissionCompleteFlag;
        if (NULL != uartHandle->tx.buffer) {
            UART_WriteByte(s_UartAdapterBase[instance],
                           uartHandle->tx.buffer[uartHandle->tx.bufferSofar++]);
            if (uartHandle->tx.bufferSofar >= uartHandle->tx.bufferLength) {
                UART_DisableInterrupts(
                    s_UartAdapterBase[uartHandle->instance],
                    (uint32_t)kUART_TxDataRegEmptyInterruptEnable);
...
hal_uart_status_t HAL_UartInit(hal_uart_handle_t handle,
    ...
    assert(config->instance <
           (sizeof(s_UartAdapterBase) / sizeof(UART_Type *)));
...
#endif
    status = UART_Init(s_UartAdapterBase[config->instance], &uartConfig,
                       config->srcClock_Hz);
    ...
    UART_TransferCreateHandle(
        s_UartAdapterBase[config->instance], &uartHandle->hardwareHandle,
        (uart_transfer_callback_t)HAL_UartCallback, handle);
#else
    s_UartState[uartHandle->instance] = uartHandle;
    /* Enable interrupt in NVIC. */
    NVIC_SetPriority((IRQn_Type)s_UartIRQ[config->instance],
...
hal_uart_status_t HAL_UartReceiveBlocking(hal_uart_handle_t handle,
    ...
    status = UART_ReadBlocking(s_UartAdapterBase[uartHandle->instance], data,
                               length);
...
hal_uart_status_t HAL_UartSendBlocking(hal_uart_handle_t handle,
    ...
    (void)UART_WriteBlocking(s_UartAdapterBase[uartHandle->instance], data,
                             length);
...
HAL_UartTransferReceiveNonBlocking(hal_uart_handle_t handle,
    ...
    status = UART_TransferReceiveNonBlocking(
        s_UartAdapterBase[uartHandle->instance], &uartHandle->hardwareHandle,
...
HAL_UartTransferSendNonBlocking(hal_uart_handle_t handle,
    ...
    status = UART_TransferSendNonBlocking(
        s_UartAdapterBase[uartHandle->instance], &uartHandle->hardwareHandle,
...
hal_uart_status_t HAL_UartTransferGetReceiveCount(hal_uart_handle_t handle,
    ...
    status =
        UART_TransferGetReceiveCount(s_UartAdapterBase[uartHandle->instance],
...
hal_uart_status_t HAL_UartTransferGetSendCount(hal_uart_handle_t handle,
    ...
    status = UART_TransferGetSendCount(s_UartAdapterBase[uartHandle->instance],
                                       &uartHandle->hardwareHandle, count);
...
hal_uart_status_t HAL_UartTransferAbortReceive(hal_uart_handle_t handle) {
    ...
    UART_TransferAbortReceive(s_UartAdapterBase[uartHandle->instance],
...
hal_uart_status_t HAL_UartTransferAbortSend(hal_uart_handle_t handle) {
    ...
    UART_TransferAbortSend(s_UartAdapterBase[uartHandle->instance],
                           &uartHandle->hardwareHandle);
...
#endif
    UART_TransferHandleIRQ(s_UartAdapterBase[uartHandle->instance],
                           &uartHandle->hardwareHandle);
#if 0
