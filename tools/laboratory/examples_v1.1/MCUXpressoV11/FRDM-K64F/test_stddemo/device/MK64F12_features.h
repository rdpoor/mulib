...
**     - rev. 2.4 (2014-01-30)
**         Added single maximum value generation and a constrain to varying
*feature values that only numbers can have maximum.
**     - rev. 2.5 (2014-02-10)
...
**     - rev. 2.9 (2015-01-21)
**         Added FSL_FEATURE_SOC_peripheral_COUNT with number of peripheral
*instances
**     - rev. 2.10 (2015-02-19)
...
/* SOC module features */
    ...
    defined(CPU_MK64FN1M0VLQ12) || defined(CPU_MK64FN1M0VMD12) ||              \
    defined(CPU_MK64FX512VDC12) || defined(CPU_MK64FX512VLQ12) ||              \
...
/* @brief Number of SC1x and Rx register pairs (conversion control and result
 * registers). */
...
/* @brief Has separate message buffer 0 interrupt flag (register bit field
 * IFLAG1[BUF0I]). */
...
/* @brief Define whether the buffer supports watermark selection detection or
 * not. */
#define FSL_FEATURE_DAC_HAS_WATERMARK_SELECTION (1)
/* @brief Define whether the buffer supports watermark event 1 word before
 * buffer upper limit. */
#define FSL_FEATURE_DAC_HAS_WATERMARK_1_WORD (1)
/* @brief Define whether the buffer supports watermark event 2 words before
 * buffer upper limit. */
#define FSL_FEATURE_DAC_HAS_WATERMARK_2_WORDS (1)
/* @brief Define whether the buffer supports watermark event 3 words before
 * buffer upper limit. */
#define FSL_FEATURE_DAC_HAS_WATERMARK_3_WORDS (1)
/* @brief Define whether the buffer supports watermark event 4 words before
 * buffer upper limit. */
...
/* @brief Has threshold for the number of frames in the receive FIFO (register
 * bit field RSEM[STAT_SECTION_EMPTY]). */
...
/* @brief FlexNVM alias start address. (Valid only if FlexNVM alias is
 * available.) */
...
#elif defined(CPU_MK64FX512VDC12) || defined(CPU_MK64FX512VLL12) ||            \
 ...
 * bytes (0xFFFF = reserved). */
#define FSL_FEATURE_FLASH_FLEX_NVM_EEPROM_SIZE_FOR_EEESIZE_1111 (0x0000)
#endif /* defined(CPU_MK64FN1M0CAJ12) || defined(CPU_MK64FN1M0VDC12) ||        \
    defined(CPU_MK64FN1M0VLL12) || defined(CPU_MK64FN1M0VLQ12) ||              \
    defined(CPU_MK64FN1M0VMD12) */
...
/* @brief If channel 6 is used to generate channel trigger, bitfield
 * EXTTRIG[CH6TRIG]. */
#define FSL_FEATURE_FTM_HAS_CHANNEL6_TRIGGER (0)
/* @brief If channel 7 is used to generate channel trigger, bitfield
 * EXTTRIG[CH7TRIG]. */
...
/* @brief Is affected by errata with ID 6070 (repeat start cannot be generated
 * if the F[MULT] bit field is set to a non-zero value). */
...
/* @brief Maximum number of pins (maximal index plus one) connected to LLWU
 * device. */
...
/* @brief Maximum PLL external reference divider value (max. value of register
 * bit field C5[PRVDIV]). */
#define FSL_FEATURE_MCG_PLL_PRDIV_MAX (24)
/* @brief VCO divider base value (multiply factor of register bit field C6[VDIV]
 * zero value). */
...
/* @brief Is PLL clock divided by 2 before MCG PLL/FLL clock selection in the
 * SIM module. */
#define FSL_FEATURE_MCG_HAS_PLL_EXTRA_DIV (0)
/* @brief Has 32kHz RTC external reference clock (register bits C8[LOCS1],
 * C8[CME1], C8[LOCRE1] and RTC module are present). */
...
/* @brief Has PLL external reference selection (register bits C5[PLLREFSEL0] and
 * C11[PLLREFSEL1]). */
#define FSL_FEATURE_MCG_USE_PLLREFSEL (0)
/* @brief TBD */
#define FSL_FEATURE_MCG_USE_SYSTEM_CLOCK (0)
/* @brief Has phase-locked loop (PLL) (register C5 and bits C6[VDIV], C6[PLLS],
 * C6[LOLIE0], S[PLLST], S[LOCK0], S[LOLS0]). */
...
/* @brief Has crystal oscillator or external reference clock low power controls
 * (register bits C2[HGO], C2[RANGE]). */
...
/* @brief Has PLL output selection (PLL0/PLL1, PLL/external PLL) (register bit
 * C11[PLLCS]). */
#define FSL_FEATURE_MCG_HAS_PLL_OUTPUT_SELECTION (0)
/* @brief Has automatic trim machine (registers ATCVH, ATCVL and bits SC[ATMF],
 * SC[ATMS], SC[ATME]). */
...
/* @brief Has low frequency internal reference clock (IRC) (registers LTRIMRNG,
 * LFRIM, LSTRIM and bit MC[LIRC_DIV2]). */
#define FSL_FEATURE_MCG_HAS_LOW_FREQ_IRC (0)
/* @brief Has high frequency internal reference clock (IRC) (registers HCTRIM,
 * HTTRIM, HFTRIM and bit MC[HIRCEN]). */
...
#if defined(CPU_MK64FN1M0CAJ12) || defined(CPU_MK64FN1M0VDC12) ||              \
    defined(CPU_MK64FN1M0VLL12) || defined(CPU_MK64FN1M0VLQ12) ||              \
    defined(CPU_MK64FX512VDC12) || defined(CPU_MK64FX512VLL12) ||              \
...
#define FSL_FEATURE_DSPI_FIFO_SIZEn(x)                                         \
    (((x) == SPI0) ? (4) : (((x) == SPI1) ? (1) : (((x) == SPI2) ? (1) : (-1))))
/* @brief Maximum transfer data width in bits. */
#define FSL_FEATURE_DSPI_MAX_DATA_WIDTH (16)
/* @brief Maximum number of chip select pins. (Reflects the width of register
 * bit field PUSHR[PCS].) */
