 ...
 *
 * Parse the contents of a JSON test file, return true if the JSON is valid.
 *
...
static bool validate_format(const char *filename) {
    ...
    if (fp == NULL) {
        ...
        // printf("\nvalidate %s\n>>>", filename);
        while (fread(&ch, sizeof(ch), sizeof(char), fp) == 1) {
            // feed a character at t time to JSON_checker_char
            // printf("%c", ch);
            if (ch == '\0') {
                // printf("--- null seen\n");
                break;
            } else if (!JSON_checker_char(jc, ch)) {
                // printf("<<< illegal json\n");
                return false;
            }
        }
        fclose(fp);
        bool success = JSON_checker_done(jc);
        // printf("<<< success = %d\n", success);
        return success;
    }
}
...
#define TEST_JSON_GOOD_FMT(f) TEST_ASSERT_TRUE_MESSAGE(validate_format(f), f);
#define TEST_JSON_BAD_FMT(f) TEST_ASSERT_FALSE_MESSAGE(validate_format(f), f);
