 ...
 *
 * Parse the contents of a JSON file, return true if expect_good is true and the
 ...
 */
static bool validate_format(const char *filename, bool expect_good) {
    char buf[200];             // longest non-pathological example is 137 bytes
    mu_json_token_t tokens[100];
    size_t max_tokens = sizeof(tokens) / sizeof(tokens[0]);
    int n_tokens;  // # of tokens parsed or negative value on bad format

    FILE *fp = fopen(filename, "r");
    if (fp == NULL) {
        printf("test error: could not open %s\n", filename);
        return false;
    } else {
        size_t n_read = fread(buf, sizeof(buf), sizeof(char), fp);
        fclose(fp);
        buf[n_read] = '\0';  // null terminate string
        n_tokens = mu_json_parse_cstr((const char *)buf, tokens, max_tokens);
        if (expect_good) {
            return n_tokens >= 0;
        } else {
            return n_tokens == MU_JSON_PARSER_BAD_FORMAT;
        }
    }
}

#define JSON_TEST_SUITE_DIR "/mnt/c/Users/r/Projects/JSONTestSuite/test_parsing/"
#define TEST_JSON_GOOD_FMT(f) TEST_ASSERT_MESSAGE(validate_format(f, true), f);
...
void test_json_validate_bad_format(void) {
...
int main(void) {
    ...
    RUN_TEST(test_json_validate_good_format);
    RUN_TEST(test_json_validate_bad_format);
