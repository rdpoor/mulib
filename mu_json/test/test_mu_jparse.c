...
static bool validate_format(const char *filename, bool expect_good) {
    ...
    mu_jparse_token_t token_store[100];
    mu_jparse_tjtree_t tree;

    mu_jparse_jtree_init(&tree, token_store,
                         sizeof(token_store) / sizeof(token_store[0]));
    ...
    if (fp == NULL) {
        ...
        fclose(fp);
        int status = mu_jparse_parse_buf(&tree, (const uint8_t *)buf, n_read);
        if (expect_good) {
            return status >= 0;
        } else {
            return status < 0;
